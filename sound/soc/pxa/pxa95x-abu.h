...
/* ABU & SSI registers*/
#define	SSICR		(0x48)	/* was SSFL -ESSI FIFO level            */
#define	SSWSS		(0x4c)	/* filler - was ESSI watermark step     */
#define	ABUCR0		(0x50)	/* ABU  Control Register                */
#define	ABUSR		(0x54)	/* ABU  Statusl Register                */
#define	ABURWR		(0x58)	/* ABU  Record Watermark Register       */
#define	ABUCR1		(0x5c)	/* ABU  Control Register                */
#define	ABUSPR		(0x84)	/* ABU  System read/write pointer       */
#define	ABUCPR		(0x88)	/* ABU  Codec read/write pointer        */
#define	ABUADSR		(0x8c)	/* ABU  Auto DMA Size Register          */
#define	ABUDR		(0x90)	/* ABU  Data Register                   */

/* ABU registers offset */
/* ABUCR0 - ABU control register 0 */
#define ABUCR0_PLAYBACK		(1 << 24)	/* 0=play/rec, 1=play only */
#define ABUCR0_CODEC_MASK	(1 << 29)	/* 0=no mask, 1=codec mask */
#define ABUCR0_SOFT_RESET	(1 << 31)	/* 0=disable, 1=enable */
#define ABUCR0_DATA_PACK_BITS	(22)		/* packing. 00 = 16b */

/* multiple of 80 packed samples */
#define ABUCR0_BUFFER_SZ_BITS	(12)
#define ABUCR0_PLAYBACK_WATERMARK_SZ_BITS	(4)

#define ABUCR0_PLAYBACK_TIMEOUT_BITS	(25)	/* playback timeout */

/* ABUCR1 - ABU control register 1 */
#define ABUCR1_BYPASS_EN	(1 << 7)	/* 0= ABU, 1 = bypass mode */
#define ABUCR1_INT_DRVN_XFR_EN	(1 << 6)	/* 1=interrupt, 0 = auto DMA */
#define ABU_AUTO_DMA_MODE	0		/* automatic DMA */
#define ABU_INTRRUPT_MODE	1		/* interrupt driven */
#define ABUCR1_TIMEOUT_INTR_EN	(1 << 5)	/* 1= enable, 0 = disable */
#define ABUCR1_ERROR_INTR_EN	(1 << 4)	/* 1= enable, 0 = disable */

/* ABURWR - ABU Record Watermark Register */
#define ABUCR0_RECORD_WATERMARK_SZ_BITS	(0)	/* multiple of 80 samples */

/* ABUADSR - ABU Auto DMA Size Register */
#define ABUADSR_PLAYBACK_SZ_BITS	(0)	/* multiple of 80 samples */
#define ABUADSR_REC_SZ_BITS		(16)	/* multiple of 80 samples */
...
/* the define for ABU logical working status */
#define ABU_DATA_PACK_16BITS		0x0
#define ABU_DATA_PACK_20BITS		0x1
#define ABU_DATA_PACK_24BITS		0x2
#define ABU_DATA_PACK_NOT_DEFINE	0x3

/* inline function */
/* ABU codec data mask in case of error */
static inline void abu_reg_set_codec_mask(void __iomem *abu_reg_base,
					  bool b_mask_flag)
{
	u32 reg_value = __raw_readl(abu_reg_base + ABUCR0);
...
static inline void abu_reg_set_playback_mode(void __iomem *abu_reg_base,
					     bool b_play_flag)
{
	u32 reg_value = __raw_readl(abu_reg_base + ABUCR0);
...
static inline void abu_reg_set_data_pack_bits(void __iomem *abu_reg_base,
					      u32 data_pack_bits)
{
	u32 reg_value = __raw_readl(abu_reg_base + ABUCR0);
...
static inline u32 abu_reg_get_data_pack_bits(void __iomem *abu_reg_base)
{
	u32 reg_value = __raw_readl(abu_reg_base + ABUCR0);
	reg_value =
	    (reg_value & (0x3 << ABUCR0_DATA_PACK_BITS)) >>
	    ABUCR0_DATA_PACK_BITS;
	return reg_value;
}

/* transfer bytes to the samples which is used to register setting directly */
static inline u8 abu_reg_bytes_to_80samples(void __iomem *abu_reg_base,
					    u32 bytes)
{
	u32 tmp = 1;
	u32 enDataPackBits;
	...
	case ABU_DATA_PACK_16BITS:
		{
			tmp = bytes / (ABU_SAMPLE_UNITS *
				     ABU_BYTES_PER_SAMPLE_16BITS);
			break;
		}
	case ABU_DATA_PACK_20BITS:
		{
			/* tmp = bytes / (ABU_SAMPLE_UNITS *
				ABU_BYTES_PER_SAMPLE_20BITS); */
			break;
		}
	default:		/* 24 bits */
		{
			/* tmp = bytes / (ABU_SAMPLE_UNITS *
				ABU_BYTES_PER_SAMPLE_24BITS); */
			break;
		}
	}
...
static inline void abu_reg_set_buffer_size(void __iomem *abu_reg_base,
					   u32 buffer_bytes)
{
...
static inline void abu_reg_set_playback_watermark(void __iomem *abu_reg_base,
						  u32 playback_watermark_bytes)
{
	u8 playback_watermark =
	    abu_reg_bytes_to_80samples(abu_reg_base, playback_watermark_bytes);
	u32 reg_value = __raw_readl(abu_reg_base + ABUCR0);
...
static inline void abu_reg_set_record_watermark(void __iomem *abu_reg_base,
						u32 record_watermark_bytes)
{
	u8 record_watermark =
	    abu_reg_bytes_to_80samples(abu_reg_base, record_watermark_bytes);
	u32 reg_value = __raw_readl(abu_reg_base + ABURWR);
...
static inline void abu_reg_set_playback_dma_size(void __iomem *abu_reg_base,
						 u32 playback_bytes)
{
	u8 playback_sz =
	    abu_reg_bytes_to_80samples(abu_reg_base, playback_bytes);
	u32 reg_value = __raw_readl(abu_reg_base + ABUADSR);
...
static inline void abu_reg_set_record_dma_size(void __iomem *abu_reg_base,
					       u32 record_bytes)
{
...
static inline void abu_reg_set_interrupt_dma_mode(void __iomem *abu_reg_base,
						  u8 intr_dma_mode)
{
	u32 reg_value = __raw_readl(abu_reg_base + ABUCR1);
...
static inline void abu_reg_set_timeout_interrupt(void __iomem *abu_reg_base,
						 bool b_flag)
{
	u32 reg_value = __raw_readl(abu_reg_base + ABUCR1);
...
static inline void abu_reg_set_error_interrupt(void __iomem *abu_reg_base,
					       bool b_flag)
{
	u32 reg_value = __raw_readl(abu_reg_base + ABUCR1);
...
static inline void abu_reg_set_playback_timeout(void __iomem *abu_reg_base,
						u8 time_out)
{
	u32 reg_value = __raw_readl(abu_reg_base + ABUCR0);
...
static inline void abu_reg_enable_disable_ssp(void __iomem *abu_reg_base,
					      bool b_flag)
{
	u32 sscr0 = __raw_readl(abu_reg_base + SSCR0);
...
{
	/* ABU SSI configure */
	/* SSI control register 0: network mode,
	   on-chip clock is selected, 16-bit data */
	/* SSI control register 1: ABU SSI work as slave mode,
	   both transimit/recevie mode */
...
{
	u32 reg_value = __raw_readl(p_abu_mmio_base + ABUSR);
	...
	if ((reg_value & ABUSR_FIFO_OVERRUN)
		...
		reg_value &=
		    (~ABUSR_FIFO_OVERRUN) & (~ABUSR_FIFO_UNDERRUN) &
		    (~ABUSR_PLAYBACKTIMEOUT);
