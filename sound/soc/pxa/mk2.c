...
static int wm8994_disable_wm8994_micb2(void)
{
	struct snd_soc_codec *codec = mmp3asoc_wm8994_codec;
	int reg;
...
static void audio_subsystem_poweron(void)
{
	...
	__raw_writel(0xd3ee2277, MPMU_ISCCRX0);
...
static int codec_wm8994_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_write(codec, WM8994_MICBIAS, 0x0044);
	snd_soc_write(codec, 0x210, 0x0037);
	snd_soc_write(codec, 0x300, 0x4010);
	snd_soc_write(codec, 0x440, 0x034b);
	snd_soc_write(codec, 0x441, 0x1253);
	snd_soc_write(codec, 0x442, 0x0810);
	snd_soc_write(codec, 0x443, 0x038c);
	snd_soc_write(codec, 0x444, 0x00a0);

#ifdef CONFIG_SWITCH_WM8994_HEADSET
	mmp3asoc_wm8994_codec = codec;
	get_mic_state = wm8994_get_mic_state;
	disable_wm8994_micb2 = wm8994_disable_wm8994_micb2;
...
{
	/* after resume back from D2 audio, someboard can 
	   not access audio sram */
	__raw_writel(__raw_readl(0xFEFFD000) | 0x60, 0xFEFFD000);
	return 0;
}
