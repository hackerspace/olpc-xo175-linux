...
static const struct snd_pcm_hardware pxa95x_pcm_abu_hardware_params = {
	...
	.period_bytes_min = 32,
	.period_bytes_max = ABU_BUFFER_SIZE - ABU_WMARk_SIZE_PLAY,
	.periods_min = 1,
...
static void pxa95x_pcm_abu_set_dvfm_constraint(int abu_dev_idx,
					       struct abu_device_ctx *p_ctx)
{
	pr_debug
	    ("[ABU] pxa95x_pcm_abu_set_dvfm_constraint (id %d) disable D1\n",
	     abu_dev_idx);
...
{
	pr_debug
	    ("[ABU] pxa95x_pcm_abu_unset_dvfm_constraint (id %d) enable D1\n",
	     abu_dev_idx);
							    ...
							    *substream)
{
	...
	if (substream) {
		...
		if (rtd)
			p_abu_dev = snd_soc_dai_get_drvdata(rtd->cpu_dai);
	}

	if (!p_abu_dev) {
		pr_err("[ABU] Can't get abu_device_ctx\n");
		return NULL;
	}
...
static u32 pxa95x_pcm_abu_clear_dma_status(int dma_ch)
{
	...
	pr_debug("[ABU] dma_ch (%d) , before clean. dcsr = 0x%x, after 0x%x\n",
...
{
	struct abu_device_ctx *p_abu_dev_rtx =
	    (struct abu_device_ctx *)p_abu_rtx;
	struct abu_dma_device *p_abu_dma_dev = &(p_abu_dev_rtx->abu_dma_dev);
	struct ssp_device *p_ssp = p_abu_dev_rtx->ssp;
	...
	for (i = 0; i < ABU_DMA_CHANNEL_NUMS; i++) {
		...
		if ((dma_ch == p_abu_dma_dev->dma_ch[i])
		    && (p_abu_dma_dev->b_stream_active[i])) {

			/* check if there was ENDINTR happened */
			if (dcsr & DCSR_ENDINTR) {

				pr_debug
				    ("[ABU] DMA (%d) ENDINTR  happened\n",
				     dma_ch);

				p_abu_dma_dev->stream_hw_params[i].
				    trans_period_nums =
				    (p_abu_dma_dev->stream_hw_params[i].
				     trans_period_nums + 1)
				    %
				...
				snd_pcm_period_elapsed(p_abu_dma_dev->
						       sub_stream[i]);

				/* if the DMA channel was in stop,
				   it can be ready for D1 mode */
				if (dcsr & DCSR_STOPSTATE) {
					DCSR(p_abu_dma_dev->dma_ch[i]) |=
					    p_abu_dma_dev->dma_dcsr[i];
					pxa95x_pcm_abu_unset_dvfm_constraint
					    (p_abu_dma_dev->dma_dvfm_idx[i],
					     p_abu_dev_rtx);
				}
			}
			...
			if (dcsr & DCSR_RASINTR) {
				...
				if ((reg_value & ABUSR_FIFO_OVERRUN)
					...
					__raw_writel(reg_value & 0x0F,
						     p_abu_mmio_base + ABUSR);
					pr_debug("[ABU] cleaned ABUSR 0x%x\n",
					     __raw_readl(p_abu_mmio_base +
							 ABUSR));

					/* reset and reinit SSP of ABU */
					abu_reg_soft_reset(p_abu_mmio_base);
					abu_reg_init_ssp(p_abu_mmio_base);
					abu_reg_enable_disable_ssp
					    (p_abu_mmio_base, true);
				}

				if (dcsr & DCSR_STOPSTATE) {

					/* start DMA transfer now,
					   it need to disable D1 mode */
					pxa95x_pcm_abu_set_dvfm_constraint
					    (p_abu_dma_dev->dma_dvfm_idx[i],
					     p_abu_dev_rtx);
...
static void pxa95x_pcm_abu_cfg_dma_desc(struct abu_device_ctx *p_abu_dev_ctx,
					u32 period_bytes, u32 period_nums,
					u32 direction, bool b_active)
{
	...
	p_abu_dma_dev->stream_hw_params[direction].total_bytes =
	    period_bytes * period_nums;
	...
	p_abu_dma_dev->stream_hw_params[direction].total_desc_nums =
	    descs_per_period * period_nums;
	p_abu_dma_dev->stream_hw_params[direction].trans_period_nums = 0;
	p_abu_dma_dev->stream_hw_params[direction].descs_per_period =
	    descs_per_period;

	pr_debug("[ABU] active %d, dir %d, p_bytes = %d, p_nums = %d\n",
	     (u32) b_active, direction, period_bytes, period_nums);
	pr_debug("[ABU] bytes_pdesc = %d, descs = %d\n",
			bytes_per_desc, descs_per_period);

	/* dma cmd width. For ABU, only support (16 bits, 16 bytes burst ) */
	...
	if (p_abu_dma_dev->sub_stream[direction])
		dma_buff_phys =
		    p_abu_dma_dev->sub_stream[direction]->dma_buffer.addr;
	else
	...
	for (i = 0; i < period_nums; i++) {
		for (j = 0; j < descs_per_period; j++) {
			...
			/* the last descriptor will pointer the head of
			   descriptor chain (circle buffer) */
			if ((i == period_nums - 1)
			    && (j == descs_per_period - 1))
				next_desc_phys =
				    p_abu_dma_dev->dma_desc_paddr[direction];
			else
			...
			if (direction == SNDRV_PCM_STREAM_PLAYBACK) {
				...
				p_dma_desc->dtadr =
				    p_abu_dma_dev->abu_dma_data[direction].
				    dev_addr;
			} else {
				p_dma_desc->dsadr =
				    p_abu_dma_dev->abu_dma_data[direction].
			...
			/* default dcmd will not enable any interrrupt per
			   descriptors. for the beginning desc of active stream
			   it need to enable start interrupt for the end desc
			   of active stream, it need to enable end interrupt */
...
{
	struct abu_device_ctx *p_abu_dev_ctx =
	    pxa95x_pcm_abu_get_ctx(substream);
	struct abu_dma_device *p_abu_dma_dev = &p_abu_dev_ctx->abu_dma_dev;
	...
	pr_debug("[ABU] stream & runtime 0x%x, 0x%x,  prt 0x%x, 0x%x, bytes %d\n",
	     (u32) substream, (u32) substream->runtime,
	     (u32) substream->runtime->dma_buffer_p,
	     (u32) substream->runtime->dma_addr,
	     (u32) substream->runtime->dma_bytes);
	...
	p_abu_dma_dev->stream_hw_params[substream->stream].period_bytes =
	    period_bytes;
	p_abu_dma_dev->stream_hw_params[substream->stream].period_nums =
	    period_nums;
...
static int pxa95x_pcm_abu_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	pr_debug("[ABU] output %d, input %d\n",
		 p_abu_dma_dev->b_stream_active[SNDRV_PCM_STREAM_PLAYBACK],
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		pxa95x_pcm_abu_set_dvfm_constraint(p_abu_dma_dev->dma_dvfm_idx[substream->stream],
						   p_abu_dev_ctx);
		...
		pxa95x_pcm_abu_cfg_dma_desc(p_abu_dev_ctx, period_bytes,
					    period_nums, substream->stream,
					    true);

		if (b_run) {
			/*  start the DMA (for any playback/recording) */
			/*  the two DMA need to be enabled even only one mode*/
			for (i = 0; i < ABU_DMA_CHANNEL_NUMS; i++) {
				DDADR(p_abu_dma_dev->dma_ch[i]) =
				    p_abu_dma_dev->dma_desc_paddr[i];
				DCSR(p_abu_dma_dev->dma_ch[i]) =
				    DCSR_RUN | p_abu_dma_dev->dma_dcsr[i];
			}

			pr_debug
			    ("[ABU] pxa95x_pcm_abu_trigger star two DMA now\n");
	...
	case SNDRV_PCM_TRIGGER_PAUSE_PUSH:
		...
		if ((!p_abu_dma_dev->b_stream_active[SNDRV_PCM_STREAM_PLAYBACK])
			...
			for (i = 0; i < ABU_DMA_CHANNEL_NUMS; i++) {
				DCSR(p_abu_dma_dev->dma_ch[i]) &=
				    (~DCSR_RUN) & (~DCSR_RASIRQEN);
			}

			pr_debug
			    ("[ABU] pxa95x_pcm_abu_trigger stop two DMA now\n");
			...
			DCSR(p_abu_dma_dev->dma_ch[substream->stream]) =
			    DCSR_RUN;

			pr_debug("[ABU] keep the DMA(%d) running DCSR 0x%x\n",
			     p_abu_dma_dev->dma_ch[substream->stream],
			     DCSR(p_abu_dma_dev->dma_ch[substream->stream]));
		...
		pxa95x_pcm_abu_unset_dvfm_constraint(p_abu_dma_dev->
						     dma_dvfm_idx[substream->
								  stream],
						     p_abu_dev_ctx);
...
static snd_pcm_uframes_t pxa95x_pcm_abu_pointer(struct snd_pcm_substream
						*substream)
{
	...
	struct abu_device_ctx *p_abu_dev_ctx =
	    pxa95x_pcm_abu_get_ctx(substream);
	struct abu_dma_device *p_abu_dma_dev = &p_abu_dev_ctx->abu_dma_dev;
	struct abu_stream_hw_params *p_params =
...
static int pxa95x_pcm_abu_open(struct snd_pcm_substream *substream)
{
	...
	snd_soc_set_runtime_hwparams(substream,
				     &pxa95x_pcm_abu_hardware_params);
	...
	ret =
	    snd_pcm_hw_constraint_step(runtime, 0,
				       SNDRV_PCM_HW_PARAM_PERIOD_BYTES, 160);
	if (ret)
		pr_err
		    ("[ABU] failed in snd_pcm_hw_constraint_step ret = %d\n",
		     ret);

	ret =
	    snd_pcm_hw_constraint_step(runtime, 0,
				       SNDRV_PCM_HW_PARAM_BUFFER_BYTES, 160);
	if (ret)
		pr_err
		    ("[ABU] failed in snd_pcm_hw_constraint_step ret = %d\n",
		     ret);
	...
	if (ret < 0)
		pr_err
		    ("[ABU] failed in snd_pcm_hw_constraint_integer ret = %d\n",
		     ret);
	...
	for (i = 0; i < ABU_DMA_CHANNEL_NUMS; i++) {
		...
		if (p_abu_dma_dev->dma_ch[i] == -1) {
			p_abu_dma_dev->abu_dma_data[i].name =
			    pxa95x_pcm_abu_channel_names[0][i];
			dma_ret =
			    pxa_request_dma(p_abu_dma_dev->abu_dma_data[i].name,
					    DMA_PRIO_LOW,
					    pxa95x_pcm_abu_dma_irq,
					    p_abu_dev_ctx);

			pr_debug("[ABU] pxa_request_dma, %d: ret is %d\n",
			     i, dma_ret);

			if (dma_ret < 0) {
				pr_err("[ABU] fail to request dma %d ret %d\n",
				     i, dma_ret);
				ret = -1;
				goto func_exit;
			}
			...
			p_abu_dma_dev->abu_dma_data[i].dev_addr =
			    p_ssp->phys_base + ABUDR;

			if (i == SNDRV_PCM_STREAM_PLAYBACK)
				p_abu_dma_dev->abu_dma_data[i].drcmr =
				    &DRCMR(p_ssp->drcmr_tx);
			else
				p_abu_dma_dev->abu_dma_data[i].drcmr =
				    &DRCMR(p_ssp->drcmr_rx);

			*(p_abu_dma_dev->abu_dma_data[i].drcmr) =
			    p_abu_dma_dev->dma_ch[i] | DRCMR_MAPVLD;
...
static int pxa95x_pcm_abu_close(struct snd_pcm_substream *substream)
{
	...
	if (p_abu_dev_ctx->abu_ref_cnt) {
		pr_debug("[ABU] pxa95x_pcm_abu_close: still stream running\n");
		return 0;
	}
...
static int pxa95x_pcm_abu_alloc_dma_buf(struct snd_pcm *pcm, int stream)
{
	...
	buf->area =
	    dma_alloc_writecombine(pcm->card->dev, size, &buf->addr,
				   GFP_KERNEL);
	if (!buf->area)
		return -ENOMEM;
	...
	pr_debug("[ABU] alloc DMA buffer size: %d, vaddr: 0x%x, paddr: 0x%x\n",
...
static void pxa95x_pcm_abu_free(struct snd_pcm *pcm)
{
	...
	struct abu_device_ctx *p_abu_rtctx =
	...
	if (p_abu_rtctx) {
		dvfm_unregister("abu-tx-dma",
				&p_abu_rtctx->abu_dma_dev.dma_dvfm_idx[0]);
		dvfm_unregister("abu-rx-dma",
...
int pxa95x_pcm_abu_new(struct snd_card *card, struct snd_soc_dai *codec_dai,
		       struct snd_pcm *pcm)
{
	...
	p_abu_rtctx = snd_soc_dai_get_drvdata(rtd->cpu_dai);
	...
	for (i = 0; i < ABU_DMA_CHANNEL_NUMS; i++) {
		...
		p_abu_dma_dev->p_dma_desc_vaddr[i] =
		    dma_alloc_writecombine(pcm->card->dev,
					   ABU_DMA_DESC_MAX_NUMS *
					   sizeof(pxa_dma_desc),
					   &(p_abu_dma_dev->dma_desc_paddr[i]),
					   GFP_KERNEL);

		if (!p_abu_dma_dev->p_dma_desc_vaddr[i]) {
			pr_err("[ABU] Failed in alloc p_dma_desc_vaddr\n");
			ret = -ENOMEM;
			goto error_out;
		}
		...
		pr_debug("[ABU] stream(%d) desc_vaddr = 0x%x, paddr = 0x%x\n",
		     i, (u32) p_abu_dma_dev->p_dma_desc_vaddr[i],
		     (u32) p_abu_dma_dev->dma_desc_paddr[i]);
	}

	/* alloca DMA buffer for playback & Recording */
	if (codec_drv->playback.channels_min) {

		pr_debug("[ABU] alloc DMA buffer for playback\n");
		ret =
		    pxa95x_pcm_abu_alloc_dma_buf(pcm,
						 SNDRV_PCM_STREAM_PLAYBACK);
		if (ret) {
			pr_err
			    ("[ABU] failed in pxa95x_pcm_abu_alloc_dma_buf\n");
			ret = -ENOMEM;
			goto error_out;
		}
	}

	if (codec_drv->capture.channels_min) {

		pr_debug("[ABU] alloc DMA buffer for recording\n");
		ret =
		    pxa95x_pcm_abu_alloc_dma_buf(pcm, SNDRV_PCM_STREAM_CAPTURE);
		if (ret) {
			pr_err
			    ("[ABU] failed in pxa95x_pcm_abu_alloc_dma_buf\n");
			ret = -ENOMEM;
	...
	if (!codec_drv->playback.channels_min
	    || !codec_drv->capture.channels_min) {
		p_abu_dma_dev->p_dummy_buf_vaddr =
		    dma_alloc_writecombine(pcm->card->dev,
					   ABU_DMA_MAX_BUFFER_BYTES,
					   &(p_abu_dma_dev->dummy_buf_paddr),
	...
	/* DVFM register */
	dvfm_register("abu-tx-dma",
		      &(p_abu_rtctx->abu_dma_dev.dma_dvfm_idx[0]));
	dvfm_register("abu-rx-dma",
		      &(p_abu_rtctx->abu_dma_dev.dma_dvfm_idx[1]));
...
struct snd_soc_platform_driver pxa95x_abu_soc_platform = {
	.ops = &pxa95x_pcm_abu_ops,
	.pcm_new = pxa95x_pcm_abu_new,
	.pcm_free = pxa95x_pcm_abu_free,
};

static int __devinit pxa95x_abu_soc_platform_probe(struct platform_device *pdev)
{
	return snd_soc_register_platform(&pdev->dev, &pxa95x_abu_soc_platform);
}

static int __devexit pxa95x_abu_soc_platform_remove(struct platform_device
						    *pdev)
{
...
static struct platform_driver pxa95x_pcm_abu_driver = {
	.driver = {
		   .name = "pxa95x-pcm-abu",
		   .owner = THIS_MODULE,
		   },

	.probe = pxa95x_abu_soc_platform_probe,
	.remove = __devexit_p(pxa95x_abu_soc_platform_remove),
};

static int __init pxa95x_pcm_abu_modinit(void)
{
	pr_info("[ABU] register abu platform driver to asoc core.\n");
	return platform_driver_register(&pxa95x_pcm_abu_driver);
}
...
static void __exit pxa95x_pcm_abu_modexit(void)
{
	...
	platform_driver_unregister(&pxa95x_pcm_abu_driver);
}
