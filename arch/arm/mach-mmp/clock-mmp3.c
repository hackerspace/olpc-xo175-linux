...
static unsigned long sdhc_clk_getrate(struct clk *clk)
{
	...
	printk("%s: APMU_SDH0 = %08X\n", __func__, __raw_readl(APMU_SDH0));
	...
	switch (pll) {
	case PLL1_div_4:
		return clk_get_rate(&mmp3_clk_pll1_d_4)/div;
	case PLL2:
		return clk_get_rate(&mmp3_clk_pll2)/div;
	case PLL1_div_2:
		return clk_get_rate(&mmp3_clk_pll1_d_2)/div;
	case PLL1:
		return clk_get_rate(&mmp3_clk_pll1)/div;
	}
	return 0;
