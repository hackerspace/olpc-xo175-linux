			       ...
			       unsigned int target_freq, unsigned int relation)
{
	...
	/* always use MP1 core as the index to do FC */
	highest_speed = mmp3_get_pp_freq(pp_index, MMP3_CLK_MP1);
#else
	highest_speed = freq_table[policy->cpu][index].frequency;
#endif
	freqs[policy->cpu].old = mmp3_cpufreq_get(policy->cpu);
	if (policy->cpu == 0 &&
		(mmp3_core_num == ONE_CORE_MM ||
		 mmp3_core_num == THREE_CORES))
		core = MMP3_CLK_MM;
		...
		core = MMP3_CLK_MP1;
	freqs[policy->cpu].new = mmp3_get_pp_freq(pp_index, core);
