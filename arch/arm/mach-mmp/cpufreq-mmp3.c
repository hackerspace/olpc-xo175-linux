...
static unsigned int prof_freq_sav_khz = 0;
...
{
	int index, i;
	unsigned int prof_freq_khz, qos_freq_khz, freq_khz;
	int ret = 0;
	...
	/* get the profiled frequency */
	prof_freq_khz = mmp3_get_pp_freq(index, MMP3_CLK_MP1);
	prof_freq_sav_khz = prof_freq_khz;
	/* get the frequency on qos min request */
	qos_freq_khz = pm_qos_request(PM_QOS_CPUFREQ_MIN) * MHZ_TO_KHZ;
	qos_freq_khz = min(qos_freq_khz, (unsigned int)mmp3_get_pp_freq(mmp3_get_pp_number() - 1, MMP3_CLK_MP1));
	freq_khz = max(prof_freq_khz, qos_freq_khz);
	/* get the frequency on qos max request */
	qos_freq_khz = pm_qos_request(PM_QOS_CPUFREQ_MAX) * MHZ_TO_KHZ;
	qos_freq_khz = max(qos_freq_khz, (unsigned int)mmp3_get_pp_freq(0, MMP3_CLK_MP1));
	freq_khz = min(freq_khz, qos_freq_khz);
...
static int cpufreq_max_notify(struct notifier_block *b,
				unsigned long max_mhz, void *v)
{
	int i;
	unsigned int max_khz, freq_khz;
	mutex_lock(&mmp3_cpu_lock);

	/* FIXME: always use MP1 core as the index to do FC */
	/* get the frequency on qos max request */
	max_khz = pm_qos_request(PM_QOS_CPUFREQ_MAX) * MHZ_TO_KHZ;
	max_khz = max(max_khz, (unsigned int)mmp3_get_pp_freq(0, MMP3_CLK_MP1));
	freq_khz = min(max_khz, prof_freq_sav_khz);
...
{
	int i;
	unsigned int min_khz, max_khz, freq_khz;
	mutex_lock(&mmp3_cpu_lock);

	/* FIXME: always use MP1 core as the index to do FC */
	min_khz = min_mhz * MHZ_TO_KHZ;
	min_khz = min(min_khz, (unsigned int)mmp3_get_pp_freq(mmp3_get_pp_number() - 1, MMP3_CLK_MP1));
	freq_khz = max(min_khz, prof_freq_sav_khz);

	/* get the frequency on qos max request */
	max_khz = pm_qos_request(PM_QOS_CPUFREQ_MAX) * MHZ_TO_KHZ;
	max_khz = max(max_khz, (unsigned int)mmp3_get_pp_freq(0, MMP3_CLK_MP1));
	freq_khz = min(freq_khz, max_khz);
				...
				unsigned long disable, void *v)
{
	...
	if (!cpufreq_disable) {
		/* get the frequency on qos min request */
		min_khz = pm_qos_request(PM_QOS_CPUFREQ_MIN) * MHZ_TO_KHZ;
		min_khz = min(min_khz, (unsigned int)mmp3_get_pp_freq(mmp3_get_pp_number() - 1, MMP3_CLK_MP1));
		freq_khz = max(min_khz, prof_freq_sav_khz);
