...
static struct pm_qos_request_list dfc_qos_idle;
...
int mmp3_cpu_killed(int cpu)
{
	u32 reg;
	reg = ((__raw_readl(APMU_REG(0x90)) >> 8) & 0x7);
	return ((reg & (1u << cpu)) == 0);
}
			...
			struct mmp3_freq_plan *pl)
{
	...
	/* allow C1 only */
	pm_qos_update_request(&dfc_qos_idle, 1);
	...
	/* allow C2 after MP2 is plugged off */
	pm_qos_update_request(&dfc_qos_idle, PM_QOS_DEFAULT_VALUE);
}
...
#if 0 //T-Lite 2, set touch power on (DIGITIZER_3V3_EN, GPIO153_GPIO153, pull high) in suspend
void  mmp3_pm_mfpr(void)
{
	unsigned int addr, gpio_value, opt;
	addr = APB_VIRT_BASE + 0x1E000+0x24c;  //SM_MEHn
	gpio_value = __raw_readl(addr);
	//printk("%s: addr[0x%x] = 0x%x.\r\n", __func__, addr, gpio_value);
	//gpio_value = __raw_writel(0x1101,addr);
	opt = ~(0x7 << 7);
	gpio_value = gpio_value & opt;
	//printk("%s: addr[0x%x] = 0x%x, opt = 0x%x.\r\n", __func__, addr, gpio_value, opt);
	opt = 0x2 << 7;
	gpio_value = gpio_value | opt;
	//printk("%s: addr[0x%x] = 0x%x, opt = 0x%x.\r\n", __func__, addr, gpio_value, opt);
	gpio_value = __raw_writel(gpio_value, addr);
	gpio_value = __raw_readl(addr);
	printk("%s: addr[0x%x] = 0x%x.\r\n", __func__, addr, gpio_value);
}
#endif //T-Lite
...
void mmp3_pm_enter_d2(void)
{
	...
	/* clear bit 13 of PCR to walk around security leak issue */
	__raw_writel(__raw_readl(0xfe051000) & ~(1 << 13), 0xfe051000);
...
//	mmp3_pm_mfpr();
	...
	__raw_writel(__raw_readl(0xfe282a44) | (0xFF << 15), 0xfe282a44);
...
static int __init mmp3_pm_init(void)
{
	...
	pm_qos_add_request(&dfc_qos_idle, PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);
