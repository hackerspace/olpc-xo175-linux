...
static unsigned long tpo_lcd_gpio_pin_config[] = {
	GPIO033_GPIO_33,  /* CS */
	GPIO034_GPIO_34,  /* Data out */
	GPIO035_GPIO_35,  /* SCLK */
	GPIO036_GPIO_36,  /* Reset */
};
...
static int gpio_spi_send_tpolcd(u16 *dat, u32 size, u32 bit_len)
{
	...
	u16 *val = dat;
	unsigned int cs_gpio = mfp_to_gpio(MFP_PIN_GPIO33);
	...
	clk_gpio = mfp_to_gpio(MFP_PIN_GPIO35);
	...
	d_gpio = mfp_to_gpio(MFP_PIN_GPIO34);
			 ...
			 unsigned int spi_gpio_reset, int on)
{
	...
	if (on) {
		...
		/* err = spi_send(fbi, tpo_spi_cmdoff,
			ARRAY_SIZE(tpo_spi_cmdoff), spi_gpio_cs); */
		if (err) {
...
static struct pxa168fb_mach_info dkb_tpo_lcd_info = {
	...
	.io_pad_ctrl		= PIN_MODE_DUMB_12_SPI_GPIO | \
				CFG_CYC_BURST_LEN8,
	.dumb_mode              = DUMB_MODE_RGB444,
	.active                 = 1,
	.spi_gpio_reset	        = mfp_to_gpio(MFP_PIN_GPIO36),
	.mmap                   = 1,
...
static struct pxa168fb_mach_info dkb_tpo_tv_info = {
	.id                     = "Base",
	.modes                  = video_modes,
	/* sclk 312M->416M */
	.sclk_div               = 0x40000029,
...
static struct pxa168fb_mach_info dkb_tpo_tv_ovly_info = {
	.id                     = "Ovly",
	.modes                  = video_modes,
	.num_modes              = ARRAY_SIZE(video_modes),
	.pix_fmt                = PIX_FMT_RGB565,
...
{
	struct pxa168fb_mach_info *fb = &dkb_tpo_tv_info;
	struct pxa168fb_mach_info *fb_ovly = &dkb_tpo_tv_ovly_info;

	/* add frame buffer drivers */
	pxa988_add_fb_tv(fb);
	pxa988_add_fb_tv_ovly(fb_ovly);
}
