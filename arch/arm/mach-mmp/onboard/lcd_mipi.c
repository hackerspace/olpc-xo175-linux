...
static int dsi_set_tc358765(struct pxa168fb_info *fbi)
{
	...
	tc35876x_write32(SYSRST, 0x00000004);
			     ...
			     unsigned int spi_gpio_reset, int on)
{
	...
	int mipi_rst, bl_en, pwm;
	mfp_cfg_t mfp_gpio53 = GPIO53_PWM3;
...
//		gpio_direction_output(mipi_rst, 0);
		...
		pwm = mfp_to_gpio(GPIO53_GPIO);
		if (gpio_request(pwm, "pwm gpio")) {
			pr_err("gpio %d request failed\n", pwm);
		}
...
pr_info("%s=================total_w %d  total_h %d\n", __func__, total_w, total_h);
...
void __init mk2_add_lcd_mipi(void)
{
	...
	bl_en = mfp_to_gpio(GPIO17_BL_EN);
        gpio_request(bl_en, "lcd bl_en gpio");
	gpio_direction_output(bl_en, 0);
	gpio_free(bl_en);
	mdelay(10);
	...
	//lvds_en = 0;
	// For TLite2, we use GPIO19 to check 
	//LVDS transmitter(MIPI) or LVDS direct out
	gpio=mfp_to_gpio(GPIO19_GPIO);  //DSI_LVDS

	if (gpio_request(gpio,"dsi lvds")){
