...
struct pm_qos_request_list cpuidle_qos_idle;
...
static int mmp3_suspend_notifier_call(struct notifier_block *nb, unsigned long state,
					void *dummy)
{
	switch (state) {
	case PM_SUSPEND_PREPARE:
		/* only allow C1 before entering D2 */
		pm_qos_update_request(&cpuidle_qos_idle, 1);
		break;
	case PM_POST_SUSPEND:
		pm_qos_update_request(&cpuidle_qos_idle, PM_QOS_DEFAULT_VALUE);
		break;
	}
	return NOTIFY_DONE;
}

static struct notifier_block mmp3_suspend_notifier = {
	.notifier_call = mmp3_suspend_notifier_call,
};
...
static int __init mmp3_cpuidle_init(void)
{
	...
	register_pm_notifier(&mmp3_suspend_notifier);
	pm_qos_add_request(&cpuidle_qos_idle, PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);
