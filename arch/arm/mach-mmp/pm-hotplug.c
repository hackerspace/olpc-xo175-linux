...
static struct pm_qos_request_list hp_qos_idle;
...
static void hotplug_timer(struct work_struct *work)
{
	...
	if (hotplug_state == HOTPLUG_DOWN) {
		if (cpu_online(1) == 1) {
			/* allow C1 only */
			pm_qos_update_request(&hp_qos_idle, 1);
			cpu_down(1);
			while (!mmp3_cpu_killed(1)) {
				cpu_relax();
			}
			/* allow C2 after MP2 is plugged off */
			pm_qos_update_request(&hp_qos_idle, PM_QOS_DEFAULT_VALUE);
		}
		goto unlock;
	}
...
static int __init mmp_pm_hotplug_init(void)
{
	...
	pm_qos_add_request(&hp_qos_idle, PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);
