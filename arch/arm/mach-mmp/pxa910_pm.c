...
{
	if (wakeup_ops && wakeup_ops->pin_restore)
...
static void pxa910_do_idle(void)
{
	...
	if (!need_resched()) {
		...
		if (awucrs & (1 << 23))
			__raw_writel(__raw_readl(MPMU_SD_ROT_WAKE_CLR) | (1 << 1),
...
static int __init pxa910_pm_init(void)
{
	...
	__raw_writel(__raw_readl(APMU_SQU_CLK_GATE_CTRL) | (1 << 30), APMU_SQU_CLK_GATE_CTRL);
