			 ...
			 int count, int *eof, void *data)
{
	...
	buf += sprintf(buf, " address:0x%x\n access: 0x%x(%d)\n id: %u\n",
		       ...
		       (unsigned int)resource->access, client_id, resource->id);
	len = buf - page;
	proc_dump_end(len, page, start, off, count, eof);
	return len;
}
...
static int group_commited(struct prm_client *client, struct prm_group *group)
{
...
static struct prm_client *get_resource_access(struct prm_resource *resource)
{
	if (resource)
		...
		return (struct prm_client *)((unsigned long)
					     prm_resources[PRM_CCNT].
					     access & (unsigned long)
					     prm_resources[PRM_PMN0].
					     access & (unsigned long)
					     prm_resources[PRM_PMN1].
					     access & (unsigned long)
					     prm_resources[PRM_PMN2].
					     access & (unsigned long)
					     prm_resources[PRM_PMN3].access);
}
...
static int __init prm_init(void)
{
	...
	for (i = 0; i < RESOURCE_NUM; i++) {
		...
		for (j = 0; j < MAX_PRIORITIES; j++) {
			...
			proc_add_resource_state(&prm_resources[i].priority[j],
						j);
		}
	}
...
static void __exit prm_exit(void)
{
	...
	for (i = 0; i < RESOURCE_NUM; i++) {
		for (j = 0; j < MAX_PRIORITIES; j++) {
			proc_del_resource_state(&prm_resources[i].priority[j],
						j);
		}
