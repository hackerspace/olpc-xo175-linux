...
static int ire_submit(struct ire_context *ctx, struct ire_submit_req submit_req)
{
	...
	if (src->y_vaddr & 63) {
		printk(KERN_ERR "IRE: the src memory base 0x%08lx is not 64 bytes align %s %d\n",
	...
	if (dst->y_vaddr & 63) {
		printk(KERN_ERR "IRE: the dst memory base 0x%08lx is not 64 bytes align %s %d\n",
...
static int ire_set_format(struct ire_context *ctx, struct ire_fmt *fmt)
{
	...
	case IRE_PIXFMT_RGB888:
		...
		if (fmt->rot_angle == IRE_ROT_180)
			yp0_pitch = y_pitch;
		break;
	/*YUV422 packed mode => YUV 444 packed*/
	case IRE_PIXFMT_YUV422_PACKED_UYVY:
	/*YUV422 packed mode => YUV 444 packed*/
	case IRE_PIXFMT_YUV422_PACKED_VYUV:
	/*YUV422 packed mode => YUV 444 packed*/
	case IRE_PIXFMT_YUV422_PACKED_YUYV:
	/*YUV422 packed mode => YUV 444 packed*/
	case IRE_PIXFMT_YUV422_PACKED_YVYU:
		y_pitch = fmt->width * 2;
		if (fmt->rot_angle == IRE_ROT_180)
			yp0_pitch = y_pitch;
		else
	...
	if ((fmt->rot_angle == IRE_ROT_90)
		&& (fmt->fmt == IRE_PIXFMT_YUV420_PLANAR))
		ctx->corner_uv = uv_pitch*(fmt->height/2-1);
	else
		ctx->corner_uv = 0;

	/* set format */
	ire_write(dev, fmt->height << 16
			| fmt->width, IRE_SIZE);/*in pixels*/
	ire_write(dev, uv_pitch << 16
			| y_pitch, IRE_PREROTATE_PITCH);/*in bytes*/
	ire_write(dev, uvp0_pitch << 16
			| yp0_pitch, IRE_POSTROTATE_PITCH);/*in bytes*/
	...
	if ((fmt->fmt == IRE_PIXFMT_YUV422_PACKED_UYVY)
		...
		if (fmt->rot_angle == IRE_ROT_180)
			ire_write(dev, fmt->fmt
				| 2 << 21 | fmt->rot_angle << 11 | 0 << 30,
		IRE_CTRL0);
		else
			ire_write(dev, fmt->fmt
				| 1 << 21 | fmt->rot_angle << 11 | 0 << 30,
		IRE_CTRL0);
	} else {
...
ire_enqueue(struct ire_context *ctx, unsigned int buf_id)
{
	...
	value |= 1 << buf_id;
	ire_write(dev, value
		| IRE_IRQ_AXI_READ_ERR | IRE_IRQ_AXI_WRITE_ERR, IRE_IRQ_MASK);
...
static long ire_ioctl(struct file *file,
...
static const struct file_operations pxa910_ire_fops = {
	.owner		= THIS_MODULE,
	.open		= ire_open,
	.release	= ire_release,
	.unlocked_ioctl	= ire_ioctl,
	.mmap		= ire_mmap,
	.fsync		= ire_fsync,
};
