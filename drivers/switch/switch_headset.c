...
static irqreturn_t headset_irq_handler(int irq, void *dev_id)
{
	...
	queue_work(switch_data->workq, &switch_data->work);
...
static int headset_switch_probe(struct platform_device *pdev)
{
	...
	switch_data->workq = create_singlethread_workqueue("headset detection queue");
	if (switch_data->workq == NULL) {
		pr_err("create work queue fail\n");
		goto err_create_workq_failed;
	}
...
err_detect_irq_num_failed:
	destroy_workqueue(switch_data->workq);
err_create_workq_failed:
