...
static int __devinit sdhci_pxav3_probe(struct platform_device *pdev)
{
	...
	pltfm_host->clk = clk;
		printk("%s/%d: APMU_SDH0 = %08X\n", __func__, __LINE__, __raw_readl(APMU_SDH0));
	...
	if (pdata) {
...
#if 0
		if (!(pdata && pdata->signal_1v8))
			host->quirks2 |= SDHCI_QUIRK2_HOST_NO_UHS_1_8V_SUPPORT;
#endif
		if (host->quirks & SDHCI_QUIRK_MISSING_CAPS) {
			...
			printk ("%s/%d: %s: CAPS = %08X\n", __func__, __LINE__, mmc_hostname(host->mmc), host->caps);
			caps_rate = (host->caps >> 8) & 0xff;
			printk("%s:%s: caps_rate = %02X or caps_rate = %d\n",
				__func__, mmc_hostname(host->mmc), caps_rate, caps_rate * 1000000);

			shift = caps_rate * 1000000/clk_get_rate(clk);
			printk ("%s/%d: %s: shift = %d\n", __func__, __LINE__, mmc_hostname(host->mmc), shift);
#if 1
			host->caps &= 0xFFFF00FF;
			host->caps |= (0xc8 >> (shift-1))<<8;
#endif
			printk ("%s: %s: CAPS = %08X\n", __func__, mmc_hostname(host->mmc), host->caps);

			host->caps1 = sdhci_readl(host, SDHCI_CAPABILITIES_1);
			printk ("%s: %s: CAPS1 = %08X\n", __func__, mmc_hostname(host->mmc), host->caps1);
			printk("%s/%d: APMU_SDH0 = %08X\n", __func__, __LINE__,__raw_readl(APMU_SDH0));
		}
