...
#include "../../../arch/arm/mach-mmp/cputype.h"
...
static int mmp3_usb_phy_init(struct phy *phy)
{
	...
	dev_info(&phy->dev, "initializing Marvell MMP3 USB PHY");

#if 0
    usb:  select crystal, not (the missing) vcxo
    
    for some reason the main PMU clock source is selected in
    mmp3_usb_phy_init_internal().  this makes it select the right thing.
-       temp |= 0xd00;
+       // temp |= 0xd00;   // Select 26 MHz VCXO clock
+       temp |= 0;          // Select crystal

// static const char *uart_parent_names[] = {"uart_pll", "vctcxo"};
all3.txt:  APMU_FSIC3_CLK_RES_CTRL             0xd4282900 0x00000D1B
yall3.txt: APMU_FSIC3_CLK_RES_CTRL             0xd4282900 0x00000D1B
...
#if 0
static int mmp3_usb_phy_exit(struct phy *phy)
{
	struct mmp3_usb_phy *mmp3_usb_phy = phy_get_drvdata(phy);
	void __iomem *base = mmp3_usb_phy->base;

	dev_info(&phy->dev, "deinitializing Marvell MMP3 USB PHY");

	if (mmp3_usb_phy->version == PXA_USB_PHY_PXA168)
...
static int mmp3_usb_phy_probe(struct platform_device *pdev)
{
	...
	mmp3_usb_phy = devm_kzalloc(dev, sizeof(struct mmp3_usb_phy), GFP_KERNEL);
	if (!mmp3_usb_phy)
		return -ENOMEM;
	...
	dev_info(dev, "Marvell MMP3 USB PHY");
	return 0;
}
