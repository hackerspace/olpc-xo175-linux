...
static int isl9226_set_enidpm(struct isl9226_device_info *info, bool enable)
{
	int ret;
	u8 value;
	ret = isl9226_read_reg(info->client, OPMOD, &value);
...
static int isl9226_set_vsppm(struct isl9226_device_info *info, u8 data)
{
	int ret;
	u8 value;
...
static int isl9226_is_charge_enabled(struct isl9226_device_info *info)
{
	int ret;
	u8 value;
	ret = isl9226_read_reg(info->client, OPMOD, &value);
...
static int isl9226_get_sta1(struct isl9226_device_info *info)
{
	int ret;
	u8 value;
				 ...
				 const struct i2c_device_id *id)
{
	...
	info->eoc_cur =
	    isl9226_get_table_data(ARRAY_AND_SIZE(eoc_current_table),
				   pdata->eoc_current);
	info->prechg_cur =
	    isl9226_get_table_data(ARRAY_AND_SIZE(prechg_current_table),
				   pdata->prechg_current);
	info->prechg_vol =
	    isl9226_get_table_data(ARRAY_AND_SIZE(prechg_voltage_table),
				   pdata->prechg_voltage);
	info->usb_chg_cur =
	    isl9226_get_table_data(ARRAY_AND_SIZE(input_current_table),
