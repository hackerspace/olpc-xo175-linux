...
#define CFG_PIN_PIN_OR_REG_CTRL			BIT(4)
...
#define CMD_B					0x31
#define CMD_B_HIGH_CURRENT_ENABLE		BIT(0)
#define CMB_B_USB500_ENABLE			BIT(1)
#define CMD_C					0x33
...
#define STAT_D					0x3e
#define STAT_D_CHARGE_MASK			0x0F
#define STAT_D_CHARGE_TYPE_OTHER		0x0B
#define STAT_D_CHARGE_TYPE_USB_SDP		0x0C
#define STAT_E					0x3f
#define STAT_E_CHARGE_MODE			(BIT(5) | BIT(6))
#define STAT_E_MODE_HIGH_CURRENT		0x00
#define STAT_E_MODE_USB500			0x40
...
static void smb347_work(struct work_struct *work)
{
	...
	smb = container_of(work, struct smb347_charger, work.work);

	printk("%s/%d: STAT_D = %02X, STAT_E = %02X\n",
		__func__, __LINE__,
		smb347_read(smb, STAT_D), smb347_read(smb, STAT_E));

	pin = smb347_read(smb, CFG_PIN);
	stat_d = smb347_read(smb, STAT_D);
	stat_e = smb347_read(smb, STAT_E);
	cmd_b = smb347_read(smb, CMD_B);
	printk("%s: pin = %02X cmd_b = %02x, stat_d = %02X, stat_e = %02X\n",
		__func__, pin, cmd_b, stat_d, stat_e);

	if (stat_d >= 0) {
		switch (stat_d & STAT_D_CHARGE_MASK) {
		case STAT_D_CHARGE_TYPE_OTHER:
			printk("%s: STAT_D_CHARGE_TYPE_OTHER\n", __func__);
			/* Power Supply Charging - Apple */
			/* enabled high current charging */
			if ((stat_e & STAT_E_CHARGE_MODE) ==
					STAT_E_MODE_HIGH_CURRENT)
				break;
			pin = smb347_read(smb, CFG_PIN);
			if (pin >= 0) {
				pin ^= CFG_PIN_PIN_OR_REG_CTRL;
				smb347_write(smb, CFG_PIN, pin);
				if (!(pin & CFG_PIN_PIN_OR_REG_CTRL)) {
					smb347_set_writable(smb, true);
					smb347_write(smb, CMD_B,
						CMD_B_HIGH_CURRENT_ENABLE);
				}
			}
			break;
		case STAT_D_CHARGE_TYPE_USB_SDP:
			printk("%s: STAT_D_CHARGE_TYPE_USB_SDP\n", __func__);
			/* USB Port -- Plugged into a USB host */
			if ((stat_e & STAT_E_CHARGE_MODE) ==
						STAT_E_MODE_USB500)
				break;
			pin = smb347_read(smb, CFG_PIN);
	...
	printk("%s/%d: PIN = %02X, CMD_B = %02X\n",
		__func__, __LINE__,
		smb347_read(smb, CFG_PIN), smb347_read(smb, CMD_B));
	printk("%s/%d: STAT_D = %02X, STAT_E = %02X\n",
		__func__, __LINE__,
		smb347_read(smb, STAT_D), smb347_read(smb, STAT_E));
	schedule_delayed_work(&smb->work, smb->workq_delay);
}
			...
			const struct i2c_device_id *id)
{
	...
	if (pdata->workq_timeout > 0) {
		smb->workq_delay = msecs_to_jiffies(pdata->workq_timeout);
		INIT_DELAYED_WORK_DEFERRABLE(&smb->work, smb347_work);
		schedule_delayed_work(&smb->work, smb->workq_delay);
	}
