...
static void send_info_frame(u8 format)
{
	...
	/* refer CEA-861-D, 6.2 Auxiliary Video Information (AVI) InfoFrame*/
	buf[0] = 0x82;
	buf[1] = 0x2;
	buf[2] = 0xd;
...
int mv_ihdmiinit(void)
{
	...
	if (!to) {
		pr_info("HDMI failed to wait video ready!\n");
	...
	if (!to) {
		pr_info("HDMI failed again to wait video ready!\n");
		get_hw_computed_timing_params(&timing);
		/* print_timings(&timing);*/
		/* print_regs(); */
		/* TODO: timeout*/
...
static int hdmitx_probe(struct platform_device *pdev)
{
	...
	hdmi_format = *(int *)pdev->dev.platform_data;
