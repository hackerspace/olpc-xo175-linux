...
static void (*adv7533_reset)(void);
...
static int adv7533_init(int fmt)
{
	...
	ret = hdmi_write(ADV7533_MAIN, ADI7533_MAIN_CEC_DSI_ADDR,
			ADV7533_I2C_ADDR_CEC_DSI);
	if (ret != 0) {
		printk(KERN_ERR "hdmi: failed to write to hdmi\n");
		return -EINVAL;
	}
	...
	const struct i2c_device_id *client_id)
{
	...
	if (client_id->driver_data == ADV7533_MAX) {
		adv7533_reset = client->dev.platform_data;
