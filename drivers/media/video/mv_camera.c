...
static struct pm_qos_request_list mv_camera_qos_ddrfreq_min;
static struct pm_qos_request_list mv_camera_qos_disable_ddrfreq;
static struct mv_camera_dev *g_mv_cam_dev;

static void set_power_constraint(struct mv_cam_pdata *cam_data)
{
	if (cam_data->qos_req_min)
		pm_qos_update_request(&mv_camera_qos_req_min,
			cam_data->qos_req_min);
	if (cam_data->qos_req_cpu_no_dfc)
		pm_qos_update_request(&mv_camera_qos_disable_cpufreq,
			cam_data->qos_req_cpu_no_dfc);
	if (cam_data->qos_req_ddr_min)
		pm_qos_update_request(&mv_camera_qos_ddrfreq_min,
			cam_data->qos_req_ddr_min);
	if (cam_data->qos_req_ddr_no_dfc)
		pm_qos_update_request(&mv_camera_qos_disable_ddrfreq,
			cam_data->qos_req_ddr_no_dfc);
}

static void unset_power_constraint(struct mv_cam_pdata *cam_data)
{
	if (cam_data->qos_req_min)
		pm_qos_update_request(&mv_camera_qos_req_min,
		...
		if (mcam->mipi_enabled == 0) {
			...
			if (board_is_mmp2_brownstone_rev5())
				...
				if (mcam->lane_num == 1)
					...
					ccic_reg_write(pcdev, REG_CSI2_CTRL0, 0x43 | 1<<22);
			mcam->mipi_enabled = 1;
		}
	} else {
...
static irqreturn_t mv_camera_irq(int irq, void *data)
{
	...
	if (irqs & IRQ_OVERFLOW)
		printk(KERN_DEBUG "irq overflow!\n");
...
static void mv_camera_remove_device(struct soc_camera_device *icd)
{
	...
	/*
	 * If I2C failure, soc_camera will not initlilaze the vb_alloc_ctx,
	 * so double free arise here.
	 */
		...
		struct v4l2_capability *cap)
{
	...
	else if (id.ident == V4L2_IDENT_OV2656)
		strcpy(cap->driver, "ov2656");
	else
...
static int __init mv_camera_init(void)
{
	...
	pm_qos_add_request(&mv_camera_qos_ddrfreq_min, PM_QOS_DDR_DEVFREQ_MIN,
			PM_QOS_DEFAULT_VALUE);
	pm_qos_add_request(&mv_camera_qos_disable_ddrfreq,
			PM_QOS_DDR_DEVFREQ_DISABLE, PM_QOS_DEFAULT_VALUE);
