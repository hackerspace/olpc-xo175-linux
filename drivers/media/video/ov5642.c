...
int ov5642_write(struct i2c_client *c, u16 reg, unsigned char value)
{
	...
	if (reg == REG_SYS && (value & SYS_RESET)) {
		...
		usleep_range(5000, 10000);  /* Wait for reset to run */
	}
	return (ret < 0) ? ret : 0;
}
...
regular:
	...
	if (get_color_effect_regs(mf->code, mf->width, mf->height)) {
		ret = ov5642_write_array(client, get_color_effect_regs(mf->code, mf->width, mf->height));
		if (ret)
			return ret;
	}
