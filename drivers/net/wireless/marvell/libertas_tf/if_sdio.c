...
static int if_sdio_enter_deep_sleep(struct lbtf_private *priv)
{
	...
	lbtf_deb_sdio("send DEEP_SLEEP command\n");
...
static void if_sdio_remove(struct sdio_func *func)
{
	...
	if (user_rmmod && (card->model == IF_SDIO_MODEL_8688)) {
		...
		lbtf_deb_sdio("send function SHUTDOWN command\n");
...
static int if_sdio_suspend(struct device *dev)
{
	...
	printk("%s: suspend: PM flags = 0x%x\n", sdio_func_id(func), flags);

	/* If we aren't being asked to wake on anything, we should bail out
	 * and let the SD stack power down the card.
	 */
	if (card->priv->wol_criteria == 0xffffffff) {
		lbs_pr_info("Suspend without wake params -- powering down card.");
		return -ENOSYS;
	}
	...
	/*
	 * FIXME: code to prepare for our becoming autonomous should go here.
	 */
...
static int if_sdio_resume(struct device *dev)
{
	...
	printk("%s: resume: we're back\n", sdio_func_id(func));

	/*
	 * FIXME: code for going back to normal operation should go here.
	 */
