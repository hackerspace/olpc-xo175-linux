 ...
 *  commands */
extern int ntrig_dispatcher_init(void);
extern void ntrig_dispatcher_exit(void);
...
static void spi_discard_raw_message(struct ntrig_spi_privdata* privdata)
{
	u16 size;
	unsigned int ret;
	ret = kfifo_out(&privdata->raw_fifo, (unsigned char*)&size, sizeof(u16));
	while(size > 0) {
		int getsize = size;
		if(getsize > MAX_SPI_TRANSFER_BUFFER_SIZE) {
			getsize = MAX_SPI_TRANSFER_BUFFER_SIZE;
		}
		ret = kfifo_out(&privdata->raw_fifo, &privdata->scratch_buf[0], getsize);
		size -= getsize;
	}
}
...
static void spi_expand_raw_fifo(struct ntrig_spi_privdata* privdata)
{
	...
	while ((get_size = kfifo_len(&privdata->raw_fifo)) > 0) {
		if (get_size > buf_size)
			get_size = buf_size;
		ret = kfifo_out(&privdata->raw_fifo, &privdata->scratch_buf[0], get_size);
...
static int spi_get_raw_data_from_fifo(struct kfifo* fifo, char* buf, size_t count)
{
	...
	if(kfifo_len(fifo)) {
		...
		ret = kfifo_out(fifo, (unsigned char*)&data_len, sizeof(u16));
		ntrig_dbg("%s: raw message size %d, count = %d\n", __FUNCTION__, data_len, count);
		if(data_len > count) {
			data_len = count;
		}
		ret = kfifo_out(fifo, buf, data_len);
	} else {
		data_len = 0;
	}
	return data_len;
...
static int register_to_dispatcher(struct spi_device* spi)
{
	...
	if(DTRG_NO_ERROR != ntrig_allocate_device(&privdata->ntrig_dispatcher)){
	...
	privdata->sensor_id = reg_ntrig_dispatcher(TYPE_BUS_SPI_HID, "spi", &ncp_func, &hid_func); 
	if (privdata->sensor_id == DTRG_FAILED) {
		ntrig_dbg("%s: Cannot register device to dispatcher\n", __FUNCTION__);
		return DTRG_FAILED;
	}
	ret = reg_ntrig_dispatcher(TYPE_BUS_SPI_RAW, "spi", &ncp_func, NULL); 
	if(ret == DTRG_FAILED) {
	...
	nd->is_touch_set = 1;
	nd->touch_width = 2;
	ntrig_create_single_touch(nd, privdata->sensor_id);
	ntrig_create_multi_touch (nd, privdata->sensor_id);
...
static int __devinit ntrig_spi_probe(struct spi_device *spi)
{
	...
	if (ntrig_dispatcher_init() < 0) {
		pr_err("%s,ntrig_dispatcher_init error\n", __func__);
		return err;
	}
...
static int ntrig_spi_remove(struct spi_device *spi)
{
	...
	un_reg_ntrig_dispatcher(pdata->ntrig_dispatcher, pdata->sensor_id, TYPE_BUS_SPI_HID, "spi");	
	un_reg_ntrig_dispatcher(pdata->ntrig_dispatcher, pdata->sensor_id, TYPE_BUS_SPI_RAW, "spi");	
