/*
 * NTrig direct event character driver
 *
...
static mr_message_types_t g_data;		// data buffer
...
static int my_open(struct inode *inode, struct file *filep)
{
	...
	if (! accept)
	{
		ntrig_dbg("ntrig-direct-event-driver inside %s: concurrent sessions unsupported - rejecting\n", __FUNCTION__);
		return -EBUSY;
	}
	ntrig_dbg("ntrig-direct-event-driver inside %s, file mode=%d, num_open_files=%d\n", __FUNCTION__, FILE_MODE(filep), num_open_files);
...
static ssize_t my_read(struct file *filep,char *buff, size_t count, loff_t *offp)
{
	...
	ntrig_dbg("ntrig-direct-event-driver inside %s\n", __FUNCTION__);
	if (count < sizeof(mr_message_types_t))	// Verify size of user buffer
	{
		ntrig_dbg("ntrig-direct-event-driver inside %s: output buffer too small\n", __FUNCTION__);
		return -EINVAL;
	}
	/* In blocking mode, wait until data is available. In polling mode, do not
	 wait; return data only if available.
	 If the file is closed while waiting for data to become available, the
	 'flush' callback will wake the 'read', which should return -EINTR (interrupted). */
	...
	if (ready)
	{
		...
		{
			ntrig_dbg("ntrig-direct-event-driver: kernel -> userspace copy failed!\n" );
			ret_count = -EFAULT;
		}
		else
		{
...
int bus_init_direct_events(void)
{
	...
	{
		ntrig_dbg("ntrig-direct-event-driver inside %s cannot start dispatcher\n", __FUNCTION__);
	...
	if (res < 0)
	{
		printk(KERN_ERR "%s: Failed to allocate chrdev region, error %d", __FUNCTION__, res);
		return res;
	}
	...
	if (res < 0)
	{
		printk(KERN_ERR "%s: Failed to add direct-event device, error %d", __FUNCTION__, res);
	...
	device_create(g_class, NULL, g_devno, "%s" ,NTRIG_DEV_NAME_DIRECT_EVENTS);
