...
#define NTRIG_SINGLE_TOUCH_DEV_NAME	"N-trig_Touch"
#define NTRIG_MULTI_TOUCH_DEV_NAME	"N-trig_Multi_Touch"
...
/* ntrig touch change the multi-touch protocol has 2 types: typeA and typeB, we used typeB in kernel 3.0 */
#define NTRIG_KERNEL_30
/**********************************************************************************/
...
__u8 			reg_ntrig_dispatcher	(int dev_type, char* bus_id, lp_ntrig_dev_ncp_func ncp_func, lp_ntrig_dev_hid_func hid_func);
...
int 			ntrig_release_single_touch	(__u8 sensor_id);
int 			ntrig_release_multi_touch	(__u8 sensor_id);

int 			ntrig_write_hid		(void * buf);

int			ntrig_setup_quirk		(message_callback push_to_quirk, 	message_callback* get_from_quirk); 
int			ntrig_setup_ncp		(message_callback_count* get_from_ncp, 	message_callback* write_to_ncp); 
...
EXPORT_SYMBOL_GPL(ntrig_allocate_device);
EXPORT_SYMBOL_GPL(reg_ntrig_dispatcher);
EXPORT_SYMBOL_GPL(un_reg_ntrig_dispatcher);
EXPORT_SYMBOL_GPL(ntrig_remove_device);
EXPORT_SYMBOL_GPL(ntrig_create_single_touch);
EXPORT_SYMBOL_GPL(ntrig_create_multi_touch	);
EXPORT_SYMBOL_GPL(ntrig_attach_single_touch);
EXPORT_SYMBOL_GPL(ntrig_attach_multi_touch	);
EXPORT_SYMBOL_GPL(ntrig_write_hid);
EXPORT_SYMBOL_GPL(ntrig_check_single_touch	);
EXPORT_SYMBOL_GPL(ntrig_check_multi_touch);
...
int read_ncp_from_device(void *buf,size_t count)
{
	...
	if (g_ncp_next_read_sensor_id >= MAX_NUMBER_OF_SENSORS) {
		ntrig_dbg("%s: wrong sensor_id: %d\n", __FUNCTION__, g_ncp_next_read_sensor_id);		
		return DTRG_FAILED;
	}

	if (g_ncp_next_read_sensor_id > LAST_DEV_TYPE) {
...
int read_dispatcher_configuration(void *buf,  int req_type) {
	...
	switch (req_type) {
		...
		case CONFIG_NUM_SENSORS:	
			...
			break;
		case CONFIG_TOUCH_SCREEN_BORDER:
			if (g_next_read_screen_border_sensor_id < MAX_NUMBER_OF_SENSORS)
			{
...
int read_sensor_from_device (void *buf) {
	...
	if (g_config_sensor_next_read_sensor_id >= MAX_NUMBER_OF_SENSORS) {
...
int read_get_bus_interface_impl (void *buf) {
	...
	if (g_get_bus_interface_read_sensor_id >= MAX_NUMBER_OF_SENSORS) {
...
{	
	...
	// NOTE: single_touch_device & multi_touch_device are released by functions ntrig_release_single_touch & ntrig_release_multi_touch
	switch (dev_type) 
	{
		case TYPE_BUS_USB_HID:
		case TYPE_BUS_SPI_HID:
...
void un_reg_ntrig_dispatcher(void *dev, __u8 sensor_id, int dev_type, char* bus_id)
{
	...
	{
		ntrig_release_single_touch(sensor_id);
		ntrig_release_multi_touch(sensor_id);
	}
...
int ntrig_create_single_touch(lp_ntrig_bus_device dev, __u8 sensor_id)
{
	...
	if (ntrig_check_single_touch(sensor_id))
		ntrig_release_single_touch(sensor_id);
...
int ntrig_create_multi_touch(lp_ntrig_bus_device dev, __u8 sensor_id)
{
	...
	/* if device was allocated before we need to release resource */		
	if (ntrig_check_multi_touch(sensor_id))
		ntrig_release_multi_touch(sensor_id);
...
int ntrig_dispatcher_init(void)
{
	int ret;
...
void ntrig_dispatcher_exit(void)
{
	bus_exit();
