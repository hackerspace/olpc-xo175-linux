	...
	struct drm_plane_state *old_state)
{
	...
	if (!old_state->visible && state->visible) {
		...
		if (drm_fb_to_armada_fb(state->fb)->fmt > CFG_420)
			val |= CFG_PDWN256x24;
		armada_reg_queue_mod(regs, idx, 0, val, dcrtc->base + LCD_SPU_SRAM_PARA1);
	}
	val = armada_src_hw(state);
	if (armada_src_hw(old_state) != val)
		armada_reg_queue_set(regs, idx, val, dcrtc->disp_regs + LCD_SPU_GRA_HPXL_VLN);
	val = armada_dst_yx(state);
	if (armada_dst_yx(old_state) != val)
		armada_reg_queue_set(regs, idx, val, dcrtc->disp_regs + LCD_SPU_GRA_OVSA_HPXL_VLN);
	val = armada_dst_hw(state);
	if (armada_dst_hw(old_state) != val)
		armada_reg_queue_set(regs, idx, val, dcrtc->disp_regs + LCD_SPU_GZM_HPXL_VLN);
	...
	struct drm_plane_state *old_state)
{
	...
	armada_reg_queue_mod(regs, idx, 0, CFG_GRA_ENA, dcrtc->dma_regs + LCD_SPU_DMA_CTRL0);
	armada_reg_queue_mod(regs, idx, CFG_PDWN256x32 | CFG_PDWN256x24 |
			     CFG_PDWN32x32 | CFG_PDWN64x66,
			     0, dcrtc->base + LCD_SPU_SRAM_PARA1);
