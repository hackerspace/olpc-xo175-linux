...
static void armada_drm_crtc_mode_set_nofb(struct drm_crtc *crtc)
{
	...
	printk("XXX MODE SET [CRTC:%d:%s] mode " DRM_MODE_FMT "\n", crtc->base.id, crtc->name, DRM_MODE_ARG(adj));
...
static struct armada_crtc * armada_drm_crtc_create(struct drm_device *drm,
	...
	const struct armada_variant *variant, struct device_node *port)
{
	...
	of_property_read_u32(port, "reg", &dcrtc->port_num);
...
armada_lcd_bind(struct device *dev, struct device *master, void *data)
{
	...
	if (!dev->of_node) {
		...
		for_each_child_of_node(parent, port) {
			dcrtc = armada_drm_crtc_create(drm, dev, base, irq, variant, port);
			if (IS_ERR(dcrtc)) {
				DRM_ERROR("Unable to create crtc for %s: %ld\n",
					  of_node_full_name(port), PTR_ERR(dcrtc));
			} else {
				block->dcrtc[num_ports++] = dcrtc;
			}
		}
