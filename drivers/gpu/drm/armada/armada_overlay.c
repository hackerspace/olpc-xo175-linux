	...
	struct drm_plane_state *old_state)
{
	...
	if (old_state->src.x1 != state->src.x1 ||
		...
		armada_reg_queue_set(regs, idx, val, dcrtc->disp_regs + LCD_SPU_DMA_PITCH_YC);
		val = armada_pitch(state, 1) << 16 | armada_pitch(state, 2);
		armada_reg_queue_set(regs, idx, val, dcrtc->disp_regs + LCD_SPU_DMA_PITCH_UV);
	...
	if ((!old_state->visible && state->visible) ||
		...
		armada_reg_queue_set(regs, idx, val, dcrtc->base + LCD_SPU_CONTRAST);
	val = armada_spu_saturation(state);
	if ((!old_state->visible && state->visible) ||
	    armada_spu_saturation(old_state) != val)
		armada_reg_queue_set(regs, idx, val, dcrtc->base + LCD_SPU_SATURATION);
	if (!old_state->visible && state->visible)
	...
	if ((!old_state->visible && state->visible) ||
		...
		armada_reg_queue_set(regs, idx, val, dcrtc->disp_regs + LCD_SPU_COLORKEY_Y);
	val = drm_to_overlay_state(state)->colorkey_ug;
	if ((!old_state->visible && state->visible) ||
	    drm_to_overlay_state(old_state)->colorkey_ug != val)
		armada_reg_queue_set(regs, idx, val, dcrtc->disp_regs + LCD_SPU_COLORKEY_U);
	val = drm_to_overlay_state(state)->colorkey_vb;
	if ((!old_state->visible && state->visible) ||
	    drm_to_overlay_state(old_state)->colorkey_vb != val)
		armada_reg_queue_set(regs, idx, val, dcrtc->disp_regs + LCD_SPU_COLORKEY_V);
	...
	if (((!old_state->visible && state->visible) ||
		...
		armada_reg_queue_mod(regs, idx, val, ADV_GRACOLORKEY |
				     ADV_VIDCOLORKEY, dcrtc->base + LCD_SPU_ADV_REG);
