 ...
 */
static unsigned pxa910_gs_buf_put(struct pxa910_gs_buf *gb, const char *buf,
				unsigned count)
{
	unsigned len;
 ...
 * Return the number of bytes copied.
 */
static unsigned pxa910_gs_buf_get(struct pxa910_gs_buf *gb, char *buf,
				unsigned count)
{
	unsigned len;
 ...
 */
static struct usb_request *pxa910_gs_alloc_req(struct usb_ep *ep, unsigned len,
					gfp_t kmalloc_flags)
{
 ...
 */
static void pxa910_gs_free_req(struct usb_ep *ep, struct usb_request *req)
{
	kfree(req->buf);
	usb_ep_free_request(ep, req);
}
...
*/
{
	...
	while (!list_empty(pool)) {
		...
		req->length = len;
		list_del(&req->list);
		req->zero = (pxa910_gs_buf_data_avail(&port->port_write_buf)
						== 0);
...
static void pxa910_gs_write_complete(struct usb_ep *ep, struct usb_request *req)
{
	struct pxa910_gs_port *port = ep->driver_data;
...
static int pxa910_gs_start_io(struct pxa910_gs_port *port)
{
	...
	status = pxa910_gs_alloc_requests(port->port_usb->in, &port->write_pool,
				   pxa910_gs_write_complete);
	if (status) {
		pxa910_gs_free_requests(ep, head);
		return status;
	}
...
static int pxa910_gs_writes_finished(struct pxa910_gs_port *p)
{
	...
	cond = (p->port_usb == NULL) || !pxa910_gs_buf_data_avail(
				&p->port_write_buf);
...
static void pxa910_gs_close(struct tty_struct *tty, struct file *file)
{
	...
	pr_debug("pxa910_gs_close: ttyGS%d (%p,%p) ...\n", port->port_num,
					tty, file);
	...
	pr_debug("pxa910_gs_close: ttyGS%d (%p,%p) done!\n",
		 port->port_num, tty, file);
...
static int pxa910_gs_write(struct tty_struct *tty, const unsigned char *buf,
					int count)
{
	struct pxa910_gs_port *port = tty->driver_data;
	unsigned long flags;
...
static int pxa910_gs_put_char(struct tty_struct *tty, unsigned char ch)
{
	...
	pr_vdebug("pxa910_gs_put_char: (%d,%p) char=0x%x, called from %p\n",
...
static void pxa910_gs_flush_chars(struct tty_struct *tty)
{
	...
	pr_vdebug("pxa910_gs_flush_chars: (%d,%p)\n", port->port_num, tty);
...
static int pxa910_gs_write_room(struct tty_struct *tty)
{
	...
	pr_vdebug("pxa910_gs_write_room: (%d,%p) room=%d\n",
...
static int pxa910_gs_chars_in_buffer(struct tty_struct *tty)
{
	...
	pr_vdebug("pxa910_gs_chars_in_buffer: (%d,%p) chars=%d\n",
...
static int pxa910_gs_break_ctl(struct tty_struct *tty, int duration)
{
	...
	pr_vdebug("pxa910_gs_break_ctl: ttyGS%d, send break (%d)\n",
 ...
 */
static int pxa910_gserial_connect(struct pxa910_gserial *gser, u8 port_num)
{
	struct pxa910_gs_port *port;
	unsigned long flags;
	int status;
	 ...
	 */
	if (port->open_count) {
		pr_debug("pxa_gserial_connect:start ttyGS%d\n", port->port_num);
		pxa910_gs_start_io(port);
		if (gser->connect)
			gser->connect(gser);
	} else {
 ...
 */
static void pxa910_gserial_disconnect(struct pxa910_gserial *gser)
{
	...
	if (port->open_count == 0 && !port->openclose)
		pxa910_gs_buf_free(&port->port_write_buf);
	pxa910_gs_free_requests(gser->out, &port->read_pool);
	pxa910_gs_free_requests(gser->out, &port->read_queue);
