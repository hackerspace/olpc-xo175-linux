...
struct pxa_f_acm {
	...
	struct pxa_acm_ep_descs fs;
	struct pxa_acm_ep_descs hs;
...
static inline struct pxa_f_acm *pxa_func_to_acm(struct usb_function *f)
{
	return container_of(f, struct pxa_f_acm, port.func);
}

static inline struct pxa_f_acm *pxa_port_to_acm(struct pxa910_gserial *p)
{
	return container_of(p, struct pxa_f_acm, port);
}
...
static struct usb_interface_assoc_descriptor pxa_acm_iad_desc = {
	.bLength = sizeof(pxa_acm_iad_desc),
	.bDescriptorType = USB_DT_INTERFACE_ASSOCIATION,
};

static struct usb_interface_descriptor pxa_acm_control_interface_desc = {
...
static struct usb_interface_descriptor pxa_acm_data_interface_desc = {
	.bLength = USB_DT_INTERFACE_SIZE,
	.bDescriptorType = USB_DT_INTERFACE,
	/* .bInterfaceNumber = DYNAMIC */
	.bNumEndpoints = 2,
...
static struct usb_cdc_header_desc pxa_acm_header_desc = {
	.bLength = sizeof(pxa_acm_header_desc),
...
pxa_acm_call_mgmt_descriptor = {
	.bLength = sizeof(pxa_acm_call_mgmt_descriptor),
	.bDescriptorType = USB_DT_CS_INTERFACE,
...
static struct usb_cdc_acm_descriptor pxa_acm_descriptor = {
	.bLength = sizeof(pxa_acm_descriptor),
	.bDescriptorType = USB_DT_CS_INTERFACE,
	.bDescriptorSubType = USB_CDC_ACM_TYPE,
	.bmCapabilities = USB_CDC_CAP_LINE,
};

static struct usb_cdc_union_desc pxa_acm_union_desc = {
	.bLength = sizeof(pxa_acm_union_desc),
	.bDescriptorType = USB_DT_CS_INTERFACE,
...
static struct usb_endpoint_descriptor pxa_acm_fs_notify_desc = {
...
static struct usb_endpoint_descriptor pxa_acm_fs_in_desc = {
	.bLength = USB_DT_ENDPOINT_SIZE,
	.bDescriptorType = USB_DT_ENDPOINT,
	.bEndpointAddress = USB_DIR_IN,
...
static struct usb_endpoint_descriptor pxa_acm_fs_out_desc = {
	.bLength = USB_DT_ENDPOINT_SIZE,
	.bDescriptorType = USB_DT_ENDPOINT,
...
static struct usb_descriptor_header *pxa_acm_fs_function[] = {
	(struct usb_descriptor_header *)&pxa_acm_iad_desc,
	(struct usb_descriptor_header *)&pxa_acm_control_interface_desc,
	(struct usb_descriptor_header *)&pxa_acm_header_desc,
	(struct usb_descriptor_header *)&pxa_acm_call_mgmt_descriptor,
	(struct usb_descriptor_header *)&pxa_acm_descriptor,
	(struct usb_descriptor_header *)&pxa_acm_union_desc,
	(struct usb_descriptor_header *)&pxa_acm_fs_notify_desc,
	(struct usb_descriptor_header *)&pxa_acm_data_interface_desc,
	(struct usb_descriptor_header *)&pxa_acm_fs_in_desc,
	(struct usb_descriptor_header *)&pxa_acm_fs_out_desc,
	NULL,
};

/* high speed support: */

static struct usb_endpoint_descriptor pxa_acm_hs_notify_desc = {
	.bLength = USB_DT_ENDPOINT_SIZE,
	.bDescriptorType = USB_DT_ENDPOINT,
	.bEndpointAddress = USB_DIR_IN,
	.bmAttributes = USB_ENDPOINT_XFER_INT,
...
static struct usb_endpoint_descriptor pxa_acm_hs_in_desc = {
	.bLength = USB_DT_ENDPOINT_SIZE,
	.bDescriptorType = USB_DT_ENDPOINT,
	.bmAttributes = USB_ENDPOINT_XFER_BULK,
...
static struct usb_endpoint_descriptor pxa_acm_hs_out_desc = {
	.bLength = USB_DT_ENDPOINT_SIZE,
	.bDescriptorType = USB_DT_ENDPOINT,
	.bmAttributes = USB_ENDPOINT_XFER_BULK,
	.wMaxPacketSize = cpu_to_le16(512),
...
static struct usb_descriptor_header *pxa_acm_hs_function[] = {
	(struct usb_descriptor_header *)&pxa_acm_iad_desc,
	(struct usb_descriptor_header *)&pxa_acm_control_interface_desc,
	(struct usb_descriptor_header *)&pxa_acm_header_desc,
	(struct usb_descriptor_header *)&pxa_acm_call_mgmt_descriptor,
	(struct usb_descriptor_header *)&pxa_acm_descriptor,
	(struct usb_descriptor_header *)&pxa_acm_union_desc,
	(struct usb_descriptor_header *)&pxa_acm_hs_notify_desc,
	(struct usb_descriptor_header *)&pxa_acm_data_interface_desc,
	(struct usb_descriptor_header *)&pxa_acm_hs_in_desc,
	(struct usb_descriptor_header *)&pxa_acm_hs_out_desc,
	NULL,
};
...
static struct usb_string pxa_acm_string_defs[] = {
	[ACM_CTRL_IDX].s = "CDC Abstract Control Model (ACM)",
	[ACM_DATA_IDX].s = "CDC ACM Data",
	{ /* ZEROES END LIST */ },
};

static struct usb_gadget_strings pxa_acm_string_table = {
	.language = 0x0409,	/* en-us */
	.strings = pxa_acm_string_defs,
};

static struct usb_gadget_strings *pxa_acm_strings[] = {
	&pxa_acm_string_table,
	NULL,
};
...
static void pxa_acm_complete_set_line_coding(struct usb_ep *ep,
...
static int pxa_acm_setup(struct usb_function *f,
			const struct usb_ctrlrequest *ctrl)
{
	...
	case ((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		req->complete = pxa_acm_complete_set_line_coding;
		break;
...
invalid:
	...
	if (value >= 0) {
		...
		if (value < 0)
			DBG(cdev, "acm response on ttyGS%d, err %d\n",
			      acm->port_num, value);
	}
...
static int pxa_acm_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	struct pxa_f_acm *acm = pxa_func_to_acm(f);
	struct usb_composite_dev *cdev = f->config->cdev;
 ...
 */
static int pxa_acm_cdc_notify(struct pxa_f_acm *acm, u8 type, u16 value,
...
static void pxa_acm_cdc_notify_complete(struct usb_ep *ep,
					struct usb_request *req)
{
	struct pxa_f_acm *acm = req->context;
	u8 doit = false;
...
static void pxa_acm_connect(struct pxa910_gserial *port)
{
	/*FIXME: Don't do anything here. move this operation to gs_ioctl*/
	/* It is used in modem initialization */

	struct pxa_f_acm *acm = pxa_port_to_acm(port);
...
static void pxa_acm_disconnect(struct pxa910_gserial *port)
{
	/*FIXME: Don't do anything here. move this operation to gs_ioctl*/
	/* It is used in modem initialization */
	struct pxa_f_acm *acm = pxa_port_to_acm(port);
...
static int pxa_acm_send_break(struct pxa910_gserial *port, int duration)
{
	struct pxa_f_acm *acm = pxa_port_to_acm(port);
	u16 state;
	...
	return pxa_acm_notify_serial_state(acm);
}

/*-------------------------------------------------------------------------*/

/* ACM function driver setup/binding */
static int pxa_acm_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	pxa_acm_control_interface_desc.bInterfaceNumber = status;
	pxa_acm_union_desc.bMasterInterface0 = status;

	/* init iad */
	pxa_acm_iad_desc.bFirstInterface = acm->ctrl_id;
	pxa_acm_iad_desc.bInterfaceCount = 0x2;
	pxa_acm_iad_desc.bFunctionClass =
	    pxa_acm_control_interface_desc.bInterfaceClass;
	pxa_acm_iad_desc.bFunctionSubClass =
	    pxa_acm_control_interface_desc.bInterfaceSubClass;
	pxa_acm_iad_desc.bFunctionProtocol =
	    pxa_acm_control_interface_desc.bInterfaceProtocol;
	...
	pxa_acm_data_interface_desc.bInterfaceNumber = status;
	pxa_acm_union_desc.bSlaveInterface0 = status;
	pxa_acm_call_mgmt_descriptor.bDataInterface = status;

	status = -ENODEV;

	/* allocate instance-specific endpoints */
	ep = usb_ep_autoconfig(cdev->gadget, &pxa_acm_fs_in_desc);
	if (!ep)
		goto fail;
	acm->port.in = ep;
	ep->driver_data = cdev;	/* claim */

	ep = usb_ep_autoconfig(cdev->gadget, &pxa_acm_fs_out_desc);
	if (!ep)
		goto fail;
	acm->port.out = ep;
	ep->driver_data = cdev;	/* claim */

	ep = usb_ep_autoconfig(cdev->gadget, &pxa_acm_fs_notify_desc);
	if (!ep)
		goto fail;
	acm->notify = ep;
	ep->driver_data = cdev;	/* claim */
	...
	acm->notify_req->complete = pxa_acm_cdc_notify_complete;
	acm->notify_req->context = acm;

	/* copy descriptors, and track endpoint copies */
	f->descriptors = usb_copy_descriptors(pxa_acm_fs_function);
	if (!f->descriptors)
		goto fail;

	acm->fs.in = usb_find_endpoint(pxa_acm_fs_function,
				       f->descriptors, &pxa_acm_fs_in_desc);
	acm->fs.out = usb_find_endpoint(pxa_acm_fs_function,
					f->descriptors, &pxa_acm_fs_out_desc);
	acm->fs.notify = usb_find_endpoint(pxa_acm_fs_function,
				f->descriptors, &pxa_acm_fs_notify_desc);
	...
	if (gadget_is_dualspeed(c->cdev->gadget)) {
		pxa_acm_hs_in_desc.bEndpointAddress =
		    pxa_acm_fs_in_desc.bEndpointAddress;
		pxa_acm_hs_out_desc.bEndpointAddress =
		    pxa_acm_fs_out_desc.bEndpointAddress;
		pxa_acm_hs_notify_desc.bEndpointAddress =
		    pxa_acm_fs_notify_desc.bEndpointAddress;

		/* copy descriptors, and track endpoint copies */
		f->hs_descriptors = usb_copy_descriptors(pxa_acm_hs_function);

		acm->hs.in = usb_find_endpoint(pxa_acm_hs_function,
					       f->hs_descriptors,
					       &pxa_acm_hs_in_desc);
		acm->hs.out =
		    usb_find_endpoint(pxa_acm_hs_function, f->hs_descriptors,
				      &pxa_acm_hs_out_desc);
		acm->hs.notify =
		    usb_find_endpoint(pxa_acm_hs_function, f->hs_descriptors,
				      &pxa_acm_hs_notify_desc);
	}
...
fail:
	...
	DBG(cdev, "%s/%p: can't bind, err %d\n", f->name, f, status);

	return status;
}

static void pxa_acm_unbind(struct usb_configuration *c, struct usb_function *f)
{
	struct pxa_f_acm *acm = pxa_func_to_acm(f);
...
static inline bool pxa_can_support_cdc(struct usb_configuration *c)
{
...
/**
 * marvell_acm_bind_config - add a CDC ACM function to a configuration
 ...
 */
int marvell_acm_bind_config(struct usb_configuration *c, u8 port_num)
{
	struct pxa_f_acm *acm;
	int status;

	if (!pxa_can_support_cdc(c))
		return -EINVAL;
	...
	if (pxa_acm_string_defs[ACM_CTRL_IDX].id == 0) {
		status = usb_string_id(c->cdev);
		if (status < 0)
			return status;
		pxa_acm_string_defs[ACM_CTRL_IDX].id = status;

		pxa_acm_control_interface_desc.iInterface = status;

		status = usb_string_id(c->cdev);
		if (status < 0)
			return status;
		pxa_acm_string_defs[ACM_DATA_IDX].id = status;

		pxa_acm_data_interface_desc.iInterface = status;
	}
	...
	acm->port.connect = pxa_acm_connect;
	acm->port.disconnect = pxa_acm_disconnect;
	acm->port.send_break = pxa_acm_send_break;

	acm->port.func.name = "acm";
	acm->port.func.strings = pxa_acm_strings;
	/* descriptors are per-instance copies */
	acm->port.func.bind = pxa_acm_bind;
	acm->port.func.unbind = pxa_acm_unbind;
	acm->port.func.set_alt = pxa_acm_set_alt;
	acm->port.func.setup = pxa_acm_setup;
	acm->port.func.disable = pxa_acm_disable;
...
int marvell_modem_gserial_setup(struct usb_gadget *g, unsigned count)
{
 ...
 */
void marvell_modem_gserial_cleanup(void)
{
	unsigned	i;
	struct pxa910_gs_port *port;
