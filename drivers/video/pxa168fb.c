...
/* ddr constraint when lcd on */
#define DDR_CONSTRAINT 533333
...
static int pxa168fb_blank(int blank, struct fb_info *info)
{
	...
	case FB_BLANK_NORMAL:
			...
			pm_qos_update_request(&fbi->qos_min_ddrfreq, PM_QOS_DEFAULT_VALUE);
			break;
	...
	case FB_BLANK_UNBLANK:
			...
			pm_qos_update_request(&fbi->qos_min_ddrfreq, DDR_CONSTRAINT);
			break;
	default:
			break;
	}
...
static int __devinit pxa168fb_probe(struct platform_device *pdev)
{
	...
	pm_qos_add_request(&fbi->qos_idle_fb, PM_QOS_CPU_DMA_LATENCY,
			PM_QOS_CONSTRAINT);
	/* init DDR devfreq qos constraint */
	pm_qos_add_request(&fbi->qos_min_ddrfreq, PM_QOS_DDR_DEVFREQ_MIN,
			PM_QOS_DEFAULT_VALUE);
	/* init wake lock */
