		      ...
		      void *file_priv)
{
	...
	printk(KERN_INFO "Enter coda7542_open()\n");
...
out:
	...
	printk(KERN_INFO "Leave coda7542_open(), ret = %d\n", ret);
	return ret;
			 ...
			 void *file_priv)
{
	...
	printk(KERN_INFO "Enter coda7542_release()\n");
	cdev = (struct uio_coda7542_dev *)info->priv;
	...
	mutex_unlock(&cdev->mutex);
	printk(KERN_INFO "Leave coda7542_release(), ret = %d\n", ret);
	return ret;
}
...
static int coda7542_probe(struct platform_device *pdev)
{
	...
	mem_dma_addr = (dma_addr_t)__virt_to_phys(mem_vir_addr);
...
#ifdef CONFIG_PM
static int coda7542_suspend(struct platform_device *dev, pm_message_t state)
{
	return 0;
}

static int coda7542_resume(struct platform_device *dev)
{
	return 0;
}
#endif
...
static int __init uio_coda7542_init(void)
{
	return platform_driver_register(&coda7542_driver);
}

static void __exit uio_coda7542_exit(void)
{
	platform_driver_unregister(&coda7542_driver);
}

module_init(uio_coda7542_init);
module_exit(uio_coda7542_exit);
