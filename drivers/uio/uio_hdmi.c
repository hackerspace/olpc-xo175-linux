...
static void set_power_constraint(struct hdmi_instance *hi, int min)
{
	...
	pm_qos_update_request(&hi->qos_ddrfreq_min, 800000);
#endif
		...
		void *file_priv)
{
	...
	case HPD_PIN_READ:
		...
		if (con_lock == FIRST_ACCESS_LOCK) {
			...
			printk(KERN_ERR "###### hpd -1\n");
		} else if (con_lock == SECOND_ACCESS_LOCK) {
			hpd = 0;
			con_lock = UNLOCK;
			printk(KERN_ERR "###### hpd 0\n");
		} else {
		...
		printk(KERN_ERR "###### late_disable_flag=%d, con_lock=%d\n", late_disable_flag, con_lock);
		printk(KERN_ERR "###### early_suspend_flag is %d, Kernel space: hpd is %d\n",
...
#if 0
	for(i=0; i<10; i++) {
                plr = __raw_readl((APB_VIRT_BASE + 0x19000) + 0x4 + 0x0);
                pdr = __raw_readl((APB_VIRT_BASE + 0x19000) + 0x4 + 0xc);
		mfp = __raw_readl((APB_VIRT_BASE + 0x1E000) + 0x280);	
		printk(KERN_ERR "#### hpd level is %d, hpi_in is %d, plr=0x%x, pdr=0x%x, mfp=0x%x\n", gpio_get_value(hi->gpio), hi->hpd_in, plr, pdr, mfp);
		mdelay(1);
	}
#endif
	if (gpio_get_value(hi->gpio) == hi->hpd_in) {
	
		printk(KERN_ERR "resume, hpd level is low, plug in\n");
		...
		schedule_delayed_work(&hi->hpd_work, msecs_to_jiffies(2000));
	} else {
#ifdef CONFIG_CPU_MMP3
		clk_enable(hi->clk);
#endif
		printk(KERN_ERR "resume, hpd level is high, plug out\n");
...
static void hdmi_delayed_func(struct work_struct *work)
{
	...
	printk(KERN_ERR "##### hdmi_delayed_func, late_disable_flag = %d\n", late_disable_flag);
...
static irqreturn_t hpd_handler(int irq, struct uio_info *dev_info)
{
	...
	printk("%s\n", __func__);
	if (timer_inited)
		mod_timer(&hi->jitter_timer, jiffies + HZ/20);
