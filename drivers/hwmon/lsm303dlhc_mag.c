...
#if 1 //T-Lite2, 
/* for that (temp solution) this globe var stores
   the driver data pointer. Because that ori codes in attrs fuctions will
   use i2c device to grab this data, but I put attrs to input_device so 
   I need this globe var to grab data */
struct lsm303dlhc_mag_data *mag_g;
#endif //T-Lite2
					...
					int *xyz)
{
	struct input_dev *input = mag->input_poll_dev->input;
        ...
        int cal[9] ={-11,150,1,83,-40,-1,73,68,99};
        int x, y, z;
        ...
        xyz[1] = (int)(x * cal[0] / 100 + y * cal[1] / 100 + z * cal[2] / 100);
        xyz[0] = (int)(x * cal[3] / 100 + y * cal[4] / 100 + z * cal[5] / 100);
        xyz[2] = (int)(x * cal[6] / 100 + y * cal[7] / 100 + z * cal[8] / 100);
				     ...
				     char *buf)
{
	...
	#if 1 //T-Lite2, 
	/* for that grabing driver data from globe
       var for fitting the ori codes */
	mag = mag_g;
    #endif //T-Lite2
...
static ssize_t attr_get_mag_data(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int xyz[3] = {0};
	int ret = 0;
...
static ssize_t lsm303dlhc_mag_write_proc(struct file *filp,
		const char *buff, size_t len, loff_t *off)
{
	char messages[256];
	int flag, ret;
	char buffer[7];
 	int lsm303dlhc_mag_pwr_en;
	unsigned int lsm303dlhc_mag_cmd;
	struct lsm303dlhc_mag_data *mag;	
...
//	printk(KERN_ERR "%s success: %s \n", __func__,messages);
	lsm303dlhc_mag_cmd = (unsigned int) simple_strtoul(messages, NULL, 10);
//	printk(KERN_ERR "%s success: ---%d---- \n", __func__,lsm303dlhc_mag_cmd);	
	switch(lsm303dlhc_mag_cmd){
	case 0:               //power off
//		printk(KERN_ERR "POWER OFF\n");
		lsm303dlhc_mag_disable(mag_g);
        	break;

	 
	case 1:               //power on
		lsm303dlhc_mag_enable(mag_g);
        	break;
...
static void create_lsm303dlhc_mag_proc_file(void)
{
	struct proc_dir_entry *lsm303dlhc_mag_proc_file = create_proc_entry("driver/lsm303dlhc_mag", 0644, NULL);
        printk(KERN_ERR "%s: n", __func__);
	if (lsm303dlhc_mag_proc_file)
		printk(KERN_ERR "%s:!!!!!!!!!!!!!! n", __func__);
	else
		printk(KERN_ERR "proc file create failed!\n");
//	lsm303dlhc_mag_proc_file->read_proc = (read_proc_t *)lsm303dlhc_mag_read_proc;
	lsm303dlhc_mag_proc_file->write_proc = (write_proc_t  *)lsm303dlhc_mag_write_proc;

}

static ssize_t lsm303dlhc_mag_interval_write_proc(struct file *filp,
		const char *buff, size_t len, loff_t *off)
{
	char messages[256];
	int flag, ret;
...
static void create_lsm303dlhc_mag_interval_proc_file(void)
{
	struct proc_dir_entry *lsm303dlhc_mag_proc_file = create_proc_entry("driver/lsm303dlhc_mag", 0644, NULL);
        printk(KERN_ERR "%s: n", __func__);
	if (lsm303dlhc_mag_proc_file) {
			   ...
			   const struct i2c_device_id *id)
{
    ...
    #if 1 //T-Lite2,
	/* for that storing the driver data into
       globe var and will be used on attrs functions */
	mag_g = mag;
    #endif //T-Lite2
	mutex_init(&mag->lock);
	mutex_lock(&mag->lock);
   ...
   create_lsm303dlhc_mag_proc_file();    
...
static int lsm303dlhc_mag_suspend(struct platform_device *pdev, pm_message_t state)
{
	IsShutdown = 0x333;
...
static int lsm303dlhc_mag_resume(struct platform_device *pdev)
{
