...
static struct early_suspend isl_early_suspend;
#endif
...
static struct work_struct ps_near_work;
static struct delayed_work ps_far_work;
...
static void check_far_work(struct work_struct *work)
{
	...
	if ((val & PROX_FLAG) == 0) {
		...
		schedule_delayed_work(&ps_far_work,
				      msecs_to_jiffies
				      (isl29043_dev.ps_interval));
		pr_debug("ps interrupt is still active\n");
	}
}

static void ps_near_work_handler(struct work_struct *work)
{
	u8 val;
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void isl29043_early_suspend(struct early_suspend *h)
{
	u8 val;
...
static void isl29043_late_resume(struct early_suspend *h)
{
	u8 val;
			  ...
			  const struct i2c_device_id *id)
{
	...
	isl29043_dev.pdata = (struct axis_sensor_platform_data *)
	...
	ret = request_irq(client->irq, isl29043_ps_irq_handler,
			  IRQF_TRIGGER_FALLING, client->name, &client->dev);
	if (ret) {
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	isl_early_suspend.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING;
	isl_early_suspend.suspend = isl29043_early_suspend;
	isl_early_suspend.resume = isl29043_late_resume;
	register_early_suspend(&isl_early_suspend);
#endif
	if (isl29043_dev.pdata && isl29043_dev.pdata->set_power)
		isl29043_dev.pdata->set_power(0);
	return 0;
...
static int __init isl29043_init(void)
{
	return i2c_add_driver(&isl29043_driver);
}

static void __exit isl29043_exit(void)
{
	i2c_del_driver(&isl29043_driver);
}
...
module_init(isl29043_init);
module_exit(isl29043_exit);
