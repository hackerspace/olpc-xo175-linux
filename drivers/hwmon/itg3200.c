...
#define ITG3200_ID_MAGIC			0x69
/* after poweron; can be changed at runtime */
...
static s32 itg3200_set_samplerate_divider(struct i2c_client *client,
		unsigned char samplerate_divider)
{
	return i2c_smbus_write_byte_data(client, ITG3200_REG_SAMPLE_RATE_DIV,
			samplerate_divider);
}
...
static s32 itg3200_set_low_pass(struct i2c_client *client,
		unsigned char low_pass)
{
	struct itg3200_data *data = i2c_get_clientdata(client);
	data->low_pass = low_pass;
	return i2c_smbus_write_byte_data(client, ITG3200_REG_LP_FULL_SCALE,
			data->low_pass | data->fullscale);
}
...
static s32 itg3200_set_fullscale(struct i2c_client *client,
		unsigned char fullscale)
{
	struct itg3200_data *data = i2c_get_clientdata(client);
...
static s32 itg3200_set_irq_config(struct i2c_client *client,
...
static s32 itg3200_set_axis_disable(struct i2c_client *client,
		unsigned char axis)
{
	struct itg3200_data *data = i2c_get_clientdata(client);
	data->powernamagement = data->powernamagement | axis;
	return i2c_smbus_write_byte_data(client, ITG3200_REG_POWER_MGMT,
			data->powernamagement);
}

static s32 itg3200_set_axis_enable(struct i2c_client *client,
		unsigned char axis)
{
	struct itg3200_data *data = i2c_get_clientdata(client);
	data->powernamagement = data->powernamagement & ~axis;
...
static s32 itg3200_read_measurements(struct i2c_client *client,
...
static ssize_t show_values(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct itg3200_sensordata data;
	struct input_dev *input = to_input_dev(dev);
	struct itg3200_data *client_data = input_get_drvdata(input);
...
static ssize_t show_samplerate(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct itg3200_data *client_data = input_get_drvdata(input);
	...
	unsigned int internal_samplerate =
		(itg3200_get_low_pass(client) == 0 ? 8000 : 1000);
	unsigned int samplerate =
		internal_samplerate/(itg3200_get_samplerate_divider(client)+1);
	return sprintf(buf, "%d\n", samplerate);
}

static ssize_t set_samplerate(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = strict_strtoul(buf, 10, &desired_samplerate);
	internal_samplerate =
		(itg3200_get_low_pass(client) == 0 ? 8000 : 1000);
...
static DEVICE_ATTR(samplerate, S_IWUSR | S_IRUGO,
		show_samplerate, set_samplerate);

static ssize_t show_low_pass(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct itg3200_data *client_data = input_get_drvdata(input);
	...
	default:
		return sprintf(buf, "invalid low pass filter setting: %d\n",
				low_pass);
	}
}

static ssize_t set_low_pass(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
	struct itg3200_data *client_data = input_get_drvdata(input);
	...
	ret = strict_strtoul(buf, 10, &param);
	if (param < 10)
...
static DEVICE_ATTR(low_pass, S_IWUSR | S_IRUGO,
		show_low_pass, set_low_pass);

static ssize_t get_enable_axis(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct itg3200_data *client_data = input_get_drvdata(input);
...
static ssize_t set_enable_axis(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int i;
	struct input_dev *input = to_input_dev(dev);
...
static DEVICE_ATTR(enable_axis, S_IWUSR | S_IRUGO,
		get_enable_axis, set_enable_axis);

static ssize_t set_disable_axis(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
	struct itg3200_data *client_data = input_get_drvdata(input);
...
static ssize_t get_clocksource(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct itg3200_data *client_data = input_get_drvdata(input);
	...
	case 0:
		chars += sprintf(buf, "internal oscillator");
		break;
	case 1:
		chars += sprintf(buf, "PLL with X Gyro reference");
		break;
	case 2:
		chars += sprintf(buf, "PLL with Y Gyro reference");
		break;
	case 3:
		chars += sprintf(buf, "PLL with Z Gyro reference");
		break;
	case 4:
		chars += sprintf(buf,
	...
	case 5:
		...
		break;
	default:
		chars += sprintf(buf, "unknown (reserved)");
		break;
	}
...
static ssize_t set_clocksource(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}
static DEVICE_ATTR(clocksource, S_IWUSR | S_IRUGO,
		get_clocksource, set_clocksource);




static ssize_t get_suspend(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct input_dev *input = to_input_dev(dev);
	struct itg3200_data *client_data = input_get_drvdata(input);
...
static ssize_t set_suspend(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct input_dev *input = to_input_dev(dev);
	struct itg3200_data *client_data = input_get_drvdata(input);
...
static int itg3200_input_init(struct itg3200_data *data)
{
			 ...
			 const struct i2c_device_id *id)
{
	...
	data->client = client;
	err = itg3200_input_init(data);

	/* Register sysfs hooks */
	err = sysfs_create_group(&data->input->dev.kobj, &itg3200_attr_group);
	if (err) {
