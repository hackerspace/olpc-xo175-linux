...
struct bmm_client_data {
	...
	u8 i2c_func_blk_rd:1;
	u8 i2c_func_blk_wr:1;
...
#ifdef BMM_SMBUS
	if (client_data && client_data->i2c_func_blk_rd) {
		...
		while (0 != len--) {
			dummy = i2c_smbus_read_byte_data(client,
					reg_addr);

			if (dummy < 0) {
				PERR("i2c read err: %d", dummy);
				return -1;
			} else {
				*data = (u8)dummy;
			}
...
static int bmm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (i2c_check_functionality(client->adapter,
		...
		client_data->i2c_func_blk_rd = 1;
	} else {
		client_data->i2c_func_blk_rd = 0;
		PINFO("i2c block read not supported");
	}
