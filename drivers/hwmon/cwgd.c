...
#define DEV_NAME		"CyWee CWGD Gyroscope"
...
#define ODR200_BW70	0x70	/* ODR = 200Hz, Bandwidth = 70Hz */
...
static char default_ctrl_regs[] = {
	ODR200_BW70 | CWGD_ENABLE_ALL_AXES | CWGD_PM_NORMAL,
	0x00,
	0x00,
#if USE_FIFO
...
static int dtime_show(struct device *dev, struct device_attribute *attr,
		      char *buf)
{
	struct i2c_client *client = to_i2c_client(dev->parent);
	struct i2c_cwgd_sensor *sensor =
			    ...
			    const char *buf, size_t size)
{
	...
	ret = cwgd_i2c_write(sensor, sensor->reg_addr, &data, 1);
	if (ret < 0)
		return ret;
			    ...
			    char *buf)
{
	...
	cwgd_i2c_read(sensor, sensor->reg_addr, &data, 1);
...
static DEVICE_ATTR(cwgd_reg_value, S_IRUGO | S_IWUSR | S_IWGRP, attr_reg_get,
		   attr_reg_set);
static DEVICE_ATTR(cwgd_reg_addr, S_IRUGO | S_IWUSR | S_IWGRP, NULL,
		   attr_addr_set);
...
static struct attribute *sysfs_attributes[] = {
	...
	&dev_attr_wake.attr,
	&dev_attr_dtime.attr,
	&dev_attr_cwgd_reg_value.attr,
	&dev_attr_cwgd_reg_addr.attr,
	NULL
};
...
static long cwgd_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CWGD_SELF_TEST:
		...
		return (long)cwgd_self_test(gyro, mode);
...
static void cwgd_work_report(struct work_struct *work)
{
	int res;
	struct i2c_cwgd_sensor *sensor =
	    container_of(work, struct i2c_cwgd_sensor, work);
		      ...
		      const struct i2c_device_id *devid)
{
	...
	if (sensor->client->irq > 0) {
		...
		disable_irq(sensor->client->irq);
		INIT_WORK(&sensor->work, cwgd_work_report);
	} else {
