...
struct lsm303dlhc_acc_data *acc_g;
#endif //T-Lite2
...
static ssize_t lsm303dlhc_acc_write_proc(struct file *filp,
		const char *buff, size_t len, loff_t *off)
{
	char messages[256];
	int flag, ret;
	char buffer[7];
 	int lsm303dlhc_acc_pwr_en;
	unsigned int lsm303dlhc_acc_cmd;	
...
//	printk(KERN_ERR "%s success: %s \n", __func__,messages);
	lsm303dlhc_acc_cmd = (unsigned int) simple_strtoul(messages, NULL, 10);
//	printk(KERN_ERR "%s success: ---%d---- \n", __func__,lsm303dlhc_acc_cmd);	
	switch(lsm303dlhc_acc_cmd){
	case 0:               //power off
//		printk(KERN_ERR "POWER OFF\n");
		lsm303dlhc_acc_disable(acc_g);
        	break;	 
	case 1:               //power on
		lsm303dlhc_acc_enable(acc_g);
        	break;
	 default:
...
static void create_lsm303dlhc_acc_proc_file(void)
{
	struct proc_dir_entry *lsm303dlhc_acc_proc_file = create_proc_entry("driver/lsm303dlhc_acc", 0644, NULL);
        printk(KERN_ERR "%s: n", __func__);
	if (lsm303dlhc_acc_proc_file) {
//		printk(KERN_ERR "%s:!!!!!!!!!!!!!! n", __func__);
	} else
		printk(KERN_ERR "proc file create failed!\n");
	//lsm303dlhc_acc_proc_file->read_proc = (read_proc_t *)lsm303dlhc_acc_read_proc;
	lsm303dlhc_acc_proc_file->write_proc = (write_proc_t  *)lsm303dlhc_acc_write_proc;
}
		...
		const struct i2c_device_id *id)
{
   ...
   create_lsm303dlhc_acc_proc_file();
...
static int lsm303dlhc_acc_suspend(struct platform_device *pdev, pm_message_t state)
{
	IsShutdown = 0x333;
...
static int lsm303dlhc_acc_resume(struct platform_device *pdev)
{
