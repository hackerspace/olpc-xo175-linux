...
static int ariel_led_probe(struct platform_device *pdev)
{
	...
	leds = devm_kcalloc(dev, 2, sizeof(*leds), GFP_KERNEL);
	if (!leds)
		return -ENOMEM;
	...
	leds[0].ec_index = EC_AMBER_LED;
	leds[0].led_cdev.name = "ariel:status:amber",
	...
	leds[1].ec_index = EC_GREEN_LED;
	leds[1].led_cdev.name = "ariel:power:green",
	...
	leds[1].led_cdev.default_trigger = "default-on";
