...
static irqreturn_t ec_input_interrupt(int irq, void *dev_id)
{
	...
	if (ec_input_read(ariel_pwrbutton, &response) < 0) {
	...
	if (response.count > ARRAY_SIZE(ec_input_response.data)) {
		response.count = ARRAY_SIZE(ec_input_response.data);
...
static int ariel_pwrbutton_probe(struct spi_device *spi)
{
	...
	priv = devm_kzalloc(&spi->dev, sizeof(struct ariel_pwrbutton), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;

	priv->client = spi;
	spi_set_drvdata(spi, ariel_pwrbutton);
