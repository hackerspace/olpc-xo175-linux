...
{ 
    ...
    if (eneec->toggle == rspdata.toggle) {
        pr_warning("WARNING: got the same toggle bit %d\n", rspdata.toggle);
        goto exit_enable_irq;
    }
...
static int eneec_probe(struct spi_device *spi)
{
    ...
    pr_info("eneec_spi_probe with modalias = %s, irq = %d\n", spi->modalias, spi->irq);
    ...
    if(eneec_read_rspdata(eneec, &rspdata) < 0) {
        pr_err("eneec_spi_read_rspdata fail \n");
        goto error_exit; 
    }
    eneec->toggle = rspdata.toggle;
    pr_debug("Drain data : 0x%04x\n", (u16) rspdata.data[1]);
...
#ifndef POLLING
    if ((err = request_irq(spi->irq, eneec_interrupt, 0, "eneec_spi", spi ))) {
        pr_err("Failed to request IRQ %d -- %d\n", spi->irq, err);
        goto error_exit;
    } else {
        pr_debug("IRQ %d requested\n", spi->irq);
        eneec->irq = spi->irq;
    }
#endif
