...
static int optic_tp_i2c_write(u8 reg, u16 val)
{
	...
	if (ret < 0)
		dev_dbg(&(g_p_optic_tp_data->input_dev->dev),
...
static int __attribute__ ((unused)) optic_tp_i2c_read_burst(u8 reg, u8 *buff, int sz)
{
	int rc;
	...
	if (rc != sz)
		printk(KERN_ERR
		       "ctec OTP read burst failed to read all bytes\n");
	return rc;
}
...
{
	input_report_key(g_p_optic_tp_data->input_dev, key, 1);	/*press */
	input_sync(g_p_optic_tp_data->input_dev);
	input_report_key(g_p_optic_tp_data->input_dev, key, 0);	/*release */
	input_sync(g_p_optic_tp_data->input_dev);
}
...
static void do_move(int x_delta, int y_delta)
{
	...
	input_report_rel(g_p_optic_tp_data->input_dev, REL_X, x_delta);
	input_report_rel(g_p_optic_tp_data->input_dev, REL_Y, y_delta);
	g_p_optic_tp_data->abs_x += x_delta;
	...
	dev_dbg(&(g_p_optic_tp_data->input_dev->dev), "abs x %d, abs y %d\n",
		g_p_optic_tp_data->abs_x, g_p_optic_tp_data->abs_y);
...
optic_tp_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (g_p_optic_tp_data->input_dev == NULL) {
		ret = -ENOMEM;
		printk(KERN_ERR
		       "optic_tp_probe: failed to allocate input device\n");
		goto err_dev_alloc;
	}
	...
	if (ret) {
		printk(KERN_ERR
		       "optic_tp_probe: Unable to register input device\n");
		ret = -EXDEV;
