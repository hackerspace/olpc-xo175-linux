...
static int wakeup_controller(int irq)
{
	...
	gpio_request(gpio, "lcd_touch_int");
...
#if 1 //T-Lite, for casing reason, I temporally change coordinates setting here, Paul @ 20110218
        PointBuf[ContactID].X = MAX_RESOLUTION - X;
        PointBuf[ContactID].Y = MAX_RESOLUTION - Y;
#else 
...
#ifdef CONFIG_TOUCHSCREEN_WACOM_W8001
	if(gpio_get_value(egalax_i2c->dualmode_gpio))
		return 0;
#endif
...
static void egalax_i2c_wq_loopback(struct work_struct *work)
{
	...
	if(MODE_WORKING==egalax_i2c->work_state)
	{
		...
		if(!atomic_read(&wait_command_ack) && --loopback_retry)
...
{
	int ret, i, retry=5, count=0;
	int gpio = irq_to_gpio(client->irq);
#ifdef CONFIG_TOUCHSCREEN_WACOM_W8001
	struct dualmode_platform_data *pdata = client->dev.platform_data;
#endif
#ifdef _FALLING_EDGE_IRQ
	u8 x_buf[MAX_I2C_LEN]={0};
#endif
...
#ifdef CONFIG_TOUCHSCREEN_WACOM_W8001
	p_egalax_i2c_dev->dualmode_gpio = pdata->gpio;
#endif
...
#ifdef _FALLING_EDGE_IRQ
	while( !gpio_get_value(gpio) && retry ){
		count = i2c_master_recv(client, x_buf, MAX_I2C_LEN);
		if(count<0)
			retry--;
	}
