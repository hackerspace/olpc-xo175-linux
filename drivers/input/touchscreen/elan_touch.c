...
static void elan_touch_work(struct work_struct *work)
{
	...
	if (elan_touch_recieve_data() < 0) {
		dev_info(&touch->i2c->dev, "%s: failed to receive data", __func__);
		return;
	}
	if (elan_buf[0] == 0x55 && elan_buf[1] == 0x55 &&
	    elan_buf[2] == 0x55 && elan_buf[3] == 0x55) {
		dev_info(&touch->i2c->dev, "elan_touch detected ok.\n");
	...
	if (elan_buf[0] == 0x52) {
		...
		if (elan_buf[1] == 0) {
			...
			for (i = 0; i < 4; i++) {
				if (i == 3)
					dev_info(&touch->i2c->dev, "0x%x \n", elan_buf[i]);
				else
					dev_info(&touch->i2c->dev, "0x%x ", elan_buf[i]);
			}
		}
...
static int index;
static ssize_t elan_reg_show(struct device *dev,
...

{
	...
	dev_info(&touch->i2c->dev, "register 0x%x: 0x%x\n", index, reg_val);
	return 0;
}

static ssize_t elan_reg_store(struct device *dev,
					...
					const char *buff, size_t len)
{
	...
	if ('w' == buff[0]) {
		elong_cmd[0] = 0x54;
		for (i = 0; i < 2; i++) {
			memcpy(vol, buff + i * 4 + 1, 4);
			reg = (int) simple_strtoul(vol, NULL, 16);
			dev_info(&touch->i2c->dev, "reg:0x%x\n", reg);
			elong_cmd[i + 1] = reg;
		}
...
static DEVICE_ATTR(reg_show, 0444, elan_reg_show, NULL);
static DEVICE_ATTR(reg_store, 0664, NULL, elan_reg_store);
...
static const struct attribute_group elan_attr_group = {
...
static void elan_touch_sleep_early_suspend(struct early_suspend *h)
{
	...
	if (ret < 0) {
		...
		dev_dbg(&touch->i2c->dev, "elan_touch enter sleep mode.\n");
...
static void elan_touch_normal_late_resume(struct early_suspend *h)
{
	...
	if (ret < 0)
		...
		dev_dbg(&touch->i2c->dev, "elan_touch reset successful.\n");
}
				      ...
				      const struct i2c_device_id *id)
{
	...
	ret = elan_touch_write(elong_reset, 4);
	if (ret < 0) {
		dev_dbg(&client->dev, "elan detect fail!\n");
		touch->i2c = NULL;
		return -ENXIO;
	} else {
		dev_dbg(&client->dev, "elan_touch reset.\n");
	}
	...
	ret = sysfs_create_group(&client->dev.kobj, &elan_attr_group);
	if (ret)
		goto out_irg;
...
{
	sysfs_remove_group(&client->dev.kobj, &elan_attr_group);
